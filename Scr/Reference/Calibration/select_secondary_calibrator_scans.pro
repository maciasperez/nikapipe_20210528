pro select_secondary_calibrator_scans, allscan_info, output_select_index, $
  ...
  if keyword_set(flux_threshold_1mm) or keyword_set(flux_threshold_2mm) then begin
     if keyword_set(flux_threshold_1mm) then min_flux_1mm = flux_threshold_1mm else $
        min_flux_1mm = -0.1
     if keyword_set(flux_threshold_2mm) then min_flux_2mm = flux_threshold_2mm else $
        min_flux_2mm = -0.5
     ...
     if nkeep gt 0 then allscan_info = allscan_info[wkeep] else begin
        ...
        print, " above ", strtrim(min_flux_1mm, 2), " at 1mm and "
        print, " above ", strtrim(min_flux_2mm, 2), " at 2mm"
        stop
     endelse
  ...
  for ical = 0, ncalibrator -1 do begin
     ...
     if ntot gt 0 then begin
        ...
        if wselect_primary[0] ge 0 then begin
           ...
           if keyword_set(output_selection_type) then output_selection_type = 'Baseline'
           ...
           if wselect_practical[0] ge 0 then begin
              ...
              if keyword_set(output_selection_type) then output_selection_type = 'Practical'
              ...
              if (keyword_set(showplot) and ntot gt 1) then begin
                 ...
                 if keyword_set(ps) then begin
                    ...
                    xyouts, index[wselect_practical]-1, 5.5, cal_info[wselect_practical].scan, charsi=0.7, orient=60
              ...
              ;; no scan of the source is selected using practical
           endif else begin
              
              print, ''
              print, 'NO SCAN SELECTED AFTER TWO TRIES !'
              print, '________________________________________________________________'
              n = n_elements(cal_info)
              for i=0, n-1 do print, i, ', scans ',cal_info[i].scan, ' at [UT] = ', cal_info[i].ut, ', FWHM_a1 = ', cal_info[i].result_fwhm_1,', FWHM_a3 = ', cal_info[i].result_fwhm_3, ', FWHM_1mm = ', strtrim(cal_info[i].result_fwhm_1mm,2),', FWHM_2mm = ', strtrim(cal_info[i].result_fwhm_2,2), ', atm transmission = ', strtrim(exp(-1.0d0*(cal_info[i].result_tau_3)/sin(cal_info[i].result_elevation_deg*!dtor)),2)
              if keyword_set(force_manual_selection) then begin
                 print, ''
                 print, 'Manual scan selection'
                 print, '____________________________________________________________'
                 print, 'If you know what you are doing, you can pick the "less bad" scan to check the photometry'
                 print, "Tips: take the one with the best combination of the following criteria: good angular resolution, high atmospheric transmission and night-time observation"
                 print, "Good luck..."
                 print, ""
                 ...
                 if strlen(chosen[0]) ge 10 then begin
                    w=where(strmatch(cal_info.scan, chosen) gt 0, n)
                    if n eq 1 then output_select_index = [output_select_index, w]
                    ;;nostop = 0
                 endif
                 if keyword_set(output_selection_type) then output_selection_type = 'Manual'
                 stop
