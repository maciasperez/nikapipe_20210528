pro check_flux_density_ratio_primary, runname, outdir, png=png, ps=ps, pdf=pdf, $
...
;;
  ...
  cal_factors[*] = recalibration_coef  
  ...
  ;;
  cal_factor_1mm = [cal_factor_1mm, replicate(cal_factors[3], n_elements(allscan_info.day))]
  cal_factor_a1 = [cal_factor_a1, replicate(cal_factors[0], n_elements(allscan_info.day))]
  cal_factor_a2 = [cal_factor_a2, replicate(cal_factors[1], n_elements(allscan_info.day))]
  cal_factor_a3 = [cal_factor_a3, replicate(cal_factors[2], n_elements(allscan_info.day))]
  ...
  if keyword_set(obstau_stability) then begin
     ...
     oplot, exp(-tau_1mm[w_select]/sin(elev[w_select])), flux_ratio_1mm[w_select], psym=cgsymcat('FILLEDCIRCLE', thick=mythick), col=col_n2r9, symsize=mysymsize 
     rms_tab[3] = stddev(flux_ratio_1mm[w_select])/mean(flux_ratio_1mm[w_select])*100.0d0
     print, 'nscan   = ', nn, nselect_tab
     print, 'factor  = ', cal_factors[3]
     ...
     oplot, exp(-tau_a1[w_select]/sin(elev[w_select])), flux_ratio_a1[w_select], psym=cgsymcat('FILLEDCIRCLE', thick=mythick), col=col_n2r9, symsize=mysymsize 
     rms_tab[0] = stddev(flux_ratio_a1[w_select])/mean(flux_ratio_a1[w_select])*100.0d0
     print, 'nscan   = ', nn, nselect_tab
        print, 'factor  = ', cal_factors[0]
     ...
     oplot, exp(-tau_a3[w_select]/sin(elev[w_select])), flux_ratio_a3[w_select], psym=cgsymcat('FILLEDCIRCLE', thick=mythick), col=col_n2r9, symsize=mysymsize 
     rms_tab[2] = stddev(flux_ratio_a3[w_select])/mean(flux_ratio_a3[w_select])*100.0d0
     print, 'nscan   = ', nn, nselect_tab
     print, 'factor  = ', cal_factors[2]
     ...
     oplot, exp(-tau_a2[w_select]/sin(elev[w_select])), flux_ratio_a2[w_select], psym=cgsymcat('FILLEDCIRCLE', thick=mythick), col=col_n2r9, symsize=mysymsize 
     rms_tab[1] = stddev(flux_ratio_a2[w_select])/mean(flux_ratio_a2[w_select])*100.0d0
     print, 'nscan   = ', nn, nselect_tab
     print, 'factor  = ', cal_factors[1]
     ...
     if savefile gt 0 then begin
        ...
        for ia = 0, 3 do begin
           printf, lun, quoi[ia], ' fact = ', cal_factors[ia], ', rms = ', rms_tab[ia]
        endfor
        close, lun
     endif
