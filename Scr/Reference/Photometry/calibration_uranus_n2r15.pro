...
pro calibration_uranus_n2r15, input_kidpar_file, nickname, $
                              compute=compute, png=png, ps=ps, $
                              outplot_dir=outplot_dir
  

  input_kidpar_file = !nika.off_proc_dir+"/kidpar_N2R15skd.fits"
  nickname          = 'N2R15v1'
  ...
  png               = 1
  ps                = 0
  outplot_dir       = !nika.plot_dir+"/N2R15/Photometry"
...
;; check if all scans were indeed processed
  ...
  for NoTauCorrect=0, check_taucorrect do begin
     ...
     wtokeep = where( fwhm[*, 0] le !nika.fwhm_nom[0 ] and $
                      fwhm[*, 1] le !nika.fwhm_nom[1] and $
                      fwhm[*, 2] le !nika.fwhm_nom[0], compl=wout, nscans, ncompl=nout)
