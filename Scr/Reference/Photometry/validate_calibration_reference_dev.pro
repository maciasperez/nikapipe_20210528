...
pro validate_calibration_reference, runname, input_kidpar_file, $
  ...
  cal_suf = ''
  if keyword_set(calibration_per_array) then cal_suf = '_calpera'
...
;;------------------------------------------------------------------------------
  ...
  if keyword_set(use_hybrid_opacity) then opacity_correction = 5 else opacity_correction = 4
...
if keyword_set(tweak_opacity) then begin
   
   restore, !nika.pipeline_dir+'/Datamanage/Tau225/modified_ATM_tau225_ratios.save'
   
   tau1_interp = modified_atm_tau225_ratio[0]*tau1
   tau2_interp = modified_atm_tau225_ratio[1]*tau2
   tau3_interp = modified_atm_tau225_ratio[2]*tau3
   tau_1mm_interp = modified_atm_tau225_ratio[3]*tau_1mm
   
   flux[*, 0] = flux[*, 0]*exp((tau1_interp-tau1)/sin(elev[*]*!dtor))
   flux[*, 1] = flux[*, 1]*exp((tau2_interp-tau2)/sin(elev[*]*!dtor))
   flux[*, 2] = flux[*, 2]*exp((tau3_interp-tau3)/sin(elev[*]*!dtor))
   flux[*, 3] = flux[*, 3]*exp((tau_1mm_interp-tau_1mm)/sin(elev[*]*!dtor))
...
if photocorr gt 0 then begin
   ...
   index = indgen(nscans)
   outplot, file='Photometric_correction_'+strtrim(source)+'_'+strtrim(nickname,2)+photocorr_suffixe, $
            png=png, ps=ps
...
;;--------------------------------------------------------------------
   ...
   oplot, fwhm[wout, 0] , flux[wout, 0], psym=7, col=0, thick=2
   ;;oplot, fwhm[wtokeep, 2] , flux[wtokeep, 2], psym=8, col=coltab[2]
   oplot, fwhm[wout, 2] , flux[wout, 2], psym=7, col=0, thick=2
   ...
   for u = 0, nut-1 do begin
      ...
      if nn gt 0 then oplot, t_1[w] , flux[w, 0], psym=4, col=(u+1.)*250./nut
      if nn gt 0 then oplot, t_3[w] , flux[w, 2], psym=8, col=(u+1.)*250./nut
   endfor
   ;;oplot, tau1[wout]/sin(elev[wout]*!dtor) , flux[wout, 0], psym=7,
   ;;col=0, thick=2
   oplot, t_1[wout] , flux[wout, 0], psym=7, col=0, thick=2
   ;;oplot, tau3[wout]/sin(elev[wout]*!dtor) , flux[wout, 2], psym=7, col=0, thick=2
   oplot, t_3[wout], flux[wout, 2], psym=7, col=0, thick=2
   oplot, [0,50], th_flux[0]*[1., 1.], col=0
   ...
   oplot, elev[wout] , flux[wout, 0], psym=7, col=0, thick=2
   ;;oplot, elev[wtokeep] , flux[wtokeep, 2], psym=8, col=coltab[2]
   oplot, elev[wout] , flux[wout, 2], psym=7, col=0, thick=2
   ...
   oplot, elev[wout] , fwhm[wout, 0], psym=7, col=0, thick=2
   ;;oplot, elev[wtokeep] , fwhm[wtokeep, 2], psym=8, col=coltab[2]
   oplot, elev[wout] , fwhm[wout, 2], psym=7, col=0, thick=2
   ...
   for u = 0, nut-1 do begin
      w=where(ut ge ut_tab[u] and ut lt ut_tab[u+1], nn)
      if nn gt 0 then oplot, tau1[w]/sin(elev[w]*!dtor) , fwhm[w, 0], psym=4, col=(u+1.)*250./nut
      if nn gt 0 then oplot, tau3[w]/sin(elev[w]*!dtor) , fwhm[w, 2], psym=8, col=(u+1.)*250./nut    
   endfor
   
   ;;oplot, tau_1mm[wtokeep] , fwhm[wtokeep, 0], psym=8, col=coltab[0]
   oplot, tau1[wout]/sin(elev[wout]*!dtor) , fwhm[wout, 0], psym=7, col=0, thick=2
   ;;oplot, tau_1mm[wtokeep] , fwhm[wtokeep, 2], psym=8, col=coltab[2]
   oplot, tau3[wout]/sin(elev[wout]*!dtor) , fwhm[wout, 2], psym=7, col=0, thick=2
   ...
   oplot, elev[wout] , tau_1mm[wout, 0], psym=7, col=0, thick=2
   ...
   oplot, fwhm[wout, 1] , flux[wout, 1], psym=7, col=0, thick=2
   ...
   for u = 0, nut-1 do begin
      w=where(ut ge ut_tab[u] and ut lt ut_tab[u+1], nn)
      if nn gt 0 then oplot, tau2[w]/sin(elev[w]*!dtor) , flux[w, 1], psym=8, col=(u+1.)*250./nut
   endfor
   ;;oplot, tau_2mm[wtokeep] , flux[wtokeep, 1], psym=8, col=coltab[1]
   oplot, tau2[wout]/sin(elev[wout]*!dtor) , flux[wout, 1], psym=7, col=0, thick=2
   oplot, [0,50], th_flux[1]*[1., 1.], col=0
   ...
   oplot, elev[wout] , flux[wout, 1], psym=7, col=0, thick=2
   ...
   oplot, elev[wout] , fwhm[wout, 1], psym=7, col=0, thick=2
   ...
   oplot, tau_2mm[wout] , fwhm[wout, 1], psym=7, col=0, thick=2
   ...
   oplot, elev[wout] , tau_2mm[wout, 0], psym=7, col=0, thick=2
   ...
   corr_file = output_dir+"/Crosscheck_calibration_"+strtrim(nickname,2)+photocorr_suffixe+cal_suf+opa_suf+'.save'
