...
pro make_geometry_5, scan_list, input_flux_th, nproc=nproc, black_list=black_list, $
                     ...
                     no_ref_center=no_ref_center, sub_kidpar_file=sub_kidpar_file, aperture_phot=aperture_phot, $
                     no_opacity_correction=no_opacity_correction, lockin_freqhigh=lockin_freqhigh
...
if iteration eq 1 then begin
   ...
   if keyword_set(decor_method) then begin
      if strupcase(decor_method) eq "RAW_MEDIAN" then begin
         maps_dir           = beam_maps_dir+"/Maps_raw_median"
         beams_output_dir   = beam_maps_dir+"/Beams_raw_median"
         kidpars_output_dir = beam_maps_dir+"/Kidpars_raw_median"
         toi_dir            = beam_maps_dir+"/TOIs_raw_median"
      endif
   endif
...
if keyword_set(prepare) then begin
   if keyword_set(simu) then begin
      ...
      if keyword_set(parallel) then begin
         geom_prepare_toi_parall, scan_list, toi_dir, maps_dir, nickname, nproc=nproc, $
                                  ...
                                  lockin_freqhigh=lockin_freqhigh
      endif else begin
         for iscan=0, n_elements(scan_list)-1 do begin
            geom_prepare_toi_sub, iscan, scan_list, toi_dir, maps_dir, nickname, nproc=nproc, $
...
if keyword_set(prepare) and multiscans eq 1 then begin
   geom_toi2kidmaps_parall, scan_list, toi_dir, maps_dir, nickname, nproc=nproc, $
                            input_kidpar_file = input_kidpar_file, reso=reso, $
                            preproc=preproc, kids_out=kids_out, $
                            sn_min_list=sn_min_list, sn_max_list=sn_max_list, $
                            zigzag=zigzag, gamma=gamma, lockin_freqhigh=lockin_freqhigh
   ;; return
endif

;; Fit beam parameters on kid maps
if keyword_set(beams) then begin
   geom_fit_beam_parameters, nproc, maps_dir, beams_output_dir, kidpars_output_dir, nickname, input_flux_th, $
...
if keyword_set(merge) then begin
   geom_merge_sub_kidpars, kidpars_output_dir, nproc, nickname, nostop=nostop, version=version, $
...
if keyword_set(select) then begin
   ...
   if keyword_set(no_ref_center) then begin
      kidpar_select_file = !nika.plot_dir+"/kidpar_"+nickname+"_v"+strtrim(version, 2)+"_NoRefCenter.fits"
   endif else begin
      kidpar_select_file = !nika.plot_dir+"/kidpar_"+nickname+"_v"+strtrim(version, 2)+".fits"
   endelse
   geom_kid_selection_2, scan_list, beams_output_dir, kidpars_output_dir, nickname, $
                         iter = iter, keep_neg = keep_neg, $
                         input_kidpar_file=kidpar_select_file, sub_kidpar_file=sub_kidpar_file
   ;; return
endif

;; Finalize Nasmyth to Azel rotation center, check ptg_numdet_ref
if keyword_set(finalize) then begin
   geom_finalize, "kidpar_select_"+strtrim(nickname,2)+".fits", ptg_numdet_ref, kidpar, version, nickname, nostop=nostop
   ;; return
endif
