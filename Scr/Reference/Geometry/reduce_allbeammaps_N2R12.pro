...
scan_list = [scan_list_uranus, scan_list_mars]
nscans    = n_elements(scan_list)

n_uranus = n_elements(scan_list_uranus)
n_mars   = n_elements(scan_list_mars)

sources   = [replicate("Uranus", n_uranus), replicate("Mars", n_mars)]
...
input_kidpar_corr2cm_file = !nika.off_proc_dir+"/kidpar_20171025s41_v2_LP_skd_kids_out.fits"
...
for iscan = 0, nscans-1 do begin
   ...
   scanlist2nickname, scan_list[iscan], nickname
...
;;-----------------------------------------------------
   if do_first_iteration gt 0 then begin
      ...
      make_geometry_5, scan_list[iscan], input_flux_th, ptg_numdet_ref=ptg_numdet_ref, iteration=iteration, $
      ...
      if show_plot gt 0 then begin
         ...
         nk_get_kidpar_ref, scan_num, day, info, kidpar_ref_file, scan=scan_list[iscan]
         kidpar_file     = "kidpar_"+scan_list[iscan]+"_v0.fits"
...
;;-------------------------------------------------------------------
   if do_second_iteration gt 0 then begin
      ...
      make_geometry_5, scan_list[iscan], input_flux_th, ptg_numdet_ref=ptg_numdet_ref, iteration=iteration, $
                       ...
                       source=source, beam_maps_dir=beam_maps_dir, input_kidpar_file=input_kidpar_file, $
                       prepare=prepare, beams=beams, merge=merge, select=select, finalize=finalize, $
                       decor_method=decor_method, decor_cm_dmin=decor_cm_dmin
      ...
      if show_plot gt 0 then begin
         ...
         nk_get_kidpar_ref, scan_num, day, info, kidpar_ref_file, scan=scan_list[iscan]
         kidpar_file     = "kidpar_"+scan_list[iscan]+"_v2.fits"
...
;;-------------------------------------------------------------------
   if do_corr2cm gt 0 then begin
      ...
      get_flatfields_pro, scan_list[iscan], input_kidpar_corr2cm_file, output_kidpar_file, $
