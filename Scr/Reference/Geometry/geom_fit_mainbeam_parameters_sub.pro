...
pro geom_fit_mainbeam_parameters_sub, i_in, file_list, maps_input_dir, maps_output_dir, kidpars_output_dir, kids_out, $
                                      nickname, $
                                      input_flux_th, $
                                      reso = reso, gamma=gamma, source=source, plateau=plateau, asymfast=asymfast, $
                                      ata_fit_beam_rmax=ata_fit_beam_rmax, $
                                      map_list_nhits_azel=map_list_nhits_azel, $
                                      map_list_nhits_nasmyth=map_list_nhits_nasmyth, $
...
if keyword_set(use_fwhm_azel) then use_fwhm_azel = 1 else use_fwhm_azel = 0
...
list_chi2_azel = 1.
beam_guess_mainbeam, map_list_azel, $
                     grid_azel.xmap, $
                     grid_azel.ymap, kidpar, $
                     ...
                     verbose=verbose, parinfo=parinfo, /noplot, $
                     method=beam_fit_method, cpu_time=cpu_time, /silent, $ ;nhits=nhits_azel, $
                     map_list_nhits=map_list_nhits_azel, chi2=list_chi2_azel, $
                     guess_fit_par=guess_fit_par, interpol=interpol
...
if use_fwhm_azel gt 0 then begin
...
;;second iteration using an annulus mask to avoid sidelobes
list_chi2_nasmyth = 1.


if use_fwhm_azel gt 0 then begin
   ;; will not be used in principle, force no interpolation
   beam_guess_mainbeam,map_list_nasmyth, $
                       grid_nasmyth.xmap, $
                       grid_nasmyth.ymap, kidpar, $
                       sidelobe_mask_r_in, sidelobe_mask_r_out, $
                       x_peaks_1, y_peaks_1, a_peaks_1, sigma_x_1, sigma_y_1, $
                       beam_list_nasmyth, theta_1, const_1, $
                       rebin=rebin_factor, $
                       verbose=verbose, parinfo=parinfo, /noplot, $
                       method=beam_fit_method, cpu_time=cpu_time, /silent, $ ;nhits=nhits_nasmyth, $
                       map_list_nhits=map_list_nhits_nasmyth, coordinates = 'nasmyth', chi2 = list_chi2_nasmyth, $
                       guess_fit_par=guess_fit_par, interpol=0
endif else begin
   ...
   beam_guess_mainbeam,map_list_nasmyth, $
                       ...
                       guess_fit_par=guess_fit_par, interpol=interpol
endelse
;; beam_guess_2nditer, map_list_azel, $
;;                     grid_azel.xmap, $
;;                     grid_azel.ymap, kidpar, $
;;                     x_peaks_1, y_peaks_1, a_peaks_1, sigma_x_1, sigma_y_1, $
;;                     beam_list_nasmyth, theta_1, const_1, $
...
if use_fwhm_azel lt 1 then begin
   kidpar.sigma_x        = sigma_x_1
   kidpar.sigma_y        = sigma_y_1
   kidpar.fwhm_x         = sigma_x_1/!fwhm2sigma
   kidpar.fwhm_y         = sigma_y_1/!fwhm2sigma
...
save, file = maps_output_dir+'/map_lists_'+nickname+"_sub_"+strtrim(i_in,2)+".save", $
      ...
      list_chi2_azel, list_chi2_nasmyth
