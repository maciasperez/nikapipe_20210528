...
pro geom_merge_sub_kidpars, kidpars_dir, nproc, nickname, nostop=nostop, version=version, raw=raw, $
...
output_kidpar_file_name = !nika.plot_dir+"/kidpar_"+nickname
...
for iarray = 1, 3 do begin
   ...
   if nw gt 2 then begin
      ...
      xyouts, kidpar[w].nas_x, kidpar[w].nas_y - (iarray eq 3)*2, $
              strtrim(kidpar[w].numdet,2), chars=0.9, col=ct[iarray-1]
      legendastro, 'Array '+strtrim([1,2,3],2), col=ct, textcol=ct, psym=1, box=0
   endif else begin
      message, /info, "There are only "+strtrim(nw,2)+" valid kids for array "+$
               strtrim(iarray,2)+" around (x,y) = (0,0) (see script for more details)"
   endelse
endfor
message, /info, "Now, check or choose the ref pixel"
if not keyword_set(ptg_numdet_ref) then begin
   print, "if not plot shows up to choose the ref pixel, then stop and report to"
   print, "Laurence Perotto or Nicolas Ponthieu."
