pro flux_density_ratio_all_v2, png=png, ps=ps, pdf=pdf, $
...
;;flux_density_ratio_all_v2, png=0, ps=1, pdf=1, obsdate_stability=0,fwhm_stability=0, obstau_stability=1, opacorr_method=2, photocorr_method = 1, nostop = 1, savefile = 0
  ...
  for irun = 0, nrun-1 do begin
     ...
     if taumeter gt 0 then begin      
        get_tau_nika_from_tau225, calib_run[irun], allscan_info.scan, tau_nika, atm=0, flux_driven=1, skydip_driven=0
     endif
  ...
  if keyword_set(obsdate_stability) then begin
     ...
     print, ' 1mm '
     print, '-----------------------'
     ymax = max( [1.35, max(flux_ratio_1mm[wsource] )]   )
     ymin = min( [0.65, min(flux_ratio_1mm[wsource])]   )
     xmax  = 0.95
     xmin  = 0.45     
     ...
     for u = 0, nut-1 do begin
        if ut_tab[u] lt ut_tab[u+1] then w=where(ut_float[wsource] ge ut_tab[u] and ut_float[wsource] lt ut_tab[u+1], nn) $
        else w=where(ut_float[wsource] ge ut_tab[u] or ut_float[wsource] lt ut_tab[u+1], nn)
     ...
     u=0
     w=where(ut_float[wsource] ge ut_tab[u] or ut_float[wsource] lt ut_tab[u+1], nn)
     oplot, exp(-tau_1mm[wsource[w]]/sin(elev[wsource[w]])), flux_ratio_1mm[wsource[w]], psym=cgsymcat('FILLEDCIRCLE', thick=0.7), col=ut_col[u], symsize=symsize
     
     ;;
     legendastro, leg, col=[ut_col], textcol=[ut_col],  box=0, charsize=charsize*0.8, pos=[0.47, ymax-(ymax-ymin)*0.1], spacing=0.9
     ;; 95% CL
     ;;-----------
     ratio = flux_ratio_1mm[wsource]
     xbin = 1
     ;; inf
     ...
     oplot, [xmin,xmax], [1., 1.]+s2_sup, col=0, linestyle=2
     oplot, [xmin,xmax], [1., 1.]-s2_inf, col=0, linestyle=2
     ...
     print, ' A2 '
     print, '-----------------------'
     ymax = max( [1.35, max(flux_ratio_a2[wsource] )]   )
     ymin = min( [0.65, min(flux_ratio_a2[wsource])]   )
     xmax  = 0.92
     xmin  = 0.60
     ...
     legendastro, leg, col=[ut_col], textcol=[ut_col],  box=0, charsize=charsize*0.8, pos=[0.62, ymax-(ymax-ymin)*0.1], spacing=0.9
     ;; 95% CL
     ;;-----------
     ratio = flux_ratio_a2[wsource]
     xbin = 1
     ;; inf
     pdf = HISTOGRAM(ratio, LOCATIONS=xbin, nbins=1000, min= 0.8, max=1.2)
     cumul = total(pdf, /cumulative)/n_elements(ratio)
  ...
  if keyword_set(obstau_stability) then begin
     ...
     touratio = [[flux_ratio_a1[wsource]], [flux_ratio_a2[wsource]], $
