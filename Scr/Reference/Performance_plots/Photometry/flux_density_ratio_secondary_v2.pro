pro flux_density_ratio_secondary_v2, png=png, ps=ps, pdf=pdf, $
  ...
  plot_suffixe = plot_suffixe+'_colorcorr'
  ...
  for irun = 0, nrun-1 do begin
     ...
     if do_photocorr gt 0 then begin
        ...
        ;                        input_delta_fwhm=input_delta_fwhm, add1mm=1 
        
        ;; photocorr demo: different photometric correction for Uranus
        ;; and for MWC349
     ...
     ;; WRITING SCAN LIST FILE
     scan_list_dir = outdir+'Scan_lists/'
     if file_test(scan_list_dir, /directory) lt 1 then spawn, "mkdir -p "+scan_list_dir
     print, ''
     scan_list_file = scan_list_dir+'List_scan_uranus_'+strtrim(string(calib_run[irun]),2)+'.save'
     print, 'Write scan list file : ',scan_list_file
     list_scan_uranus = allscan_info[wu].scan
     save, list_scan_uranus, file=scan_list_file
  ...
  ;; save scan list secondary
  print, ''
  w_total = where(obj eq 'MWC349', n_total)
  wsource = where(index_select gt 0 and obj eq 'MWC349')
  for irun=0, nrun-1 do begin
     print, ''
     print, calib_run[irun]
     wt = where(runid[w_total] eq calib_run[irun], nt)
     print, 'nscan = ', nt
     w = where(runid[wsource] eq calib_run[irun], nn)
     wmwc = wsource[w]
     scan_list_file = scan_list_dir+'List_scan_MWC349_'+strtrim(string(calib_run[irun]),2)+'.save'
     print, 'Write scan list file : ',scan_list_file
     print, 'N scans = ', nn
     list_scan_mwc349 = scan_list[wmwc] 
     save, list_scan_mwc349, file=scan_list_file
  endfor
  ...
  if keyword_set(mwc349_stability) then begin
     ...
     for irun=0, nrun-1 do begin
        ...
        w = where(runid[wsource] eq calib_run[irun], nn)
        ww = wsource[w]
        if nn gt 0 then oplot, exp(-tau_1mm[ww]/sin(elev[ww])), flux_ratio_1mm[ww], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25), col=col_tab[irun]
        ...
        if nflag gt 0 then oplot, exp(-tau_1mm[wt[wflag]]/sin(elev[wt[wflag]])), flux_ratio_1mm[wt[wflag]], $
                                  psym=cgsymcat('OPENCIRCLE', thick=thick*0.25), col=col_tab[irun]
        print, 'nscan = ', nn
        nselect_tab[irun] = nn
        bias = mean(flux_ratio_1mm[ww])
        rms  = stddev(flux_ratio_1mm[ww])/mean(flux_ratio_1mm[ww])*100
        print, 'bias = ', bias
        bias_tab[3, irun] =  bias
        print, 'rel.rms = ',rms
        rms_tab[3, irun] = rms
     endfor
  
     ;;
     legendastro, calib_run, col=col_tab, psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25)*[1., 1., 1.], textcol=0, box=0, charsize=charsize, pos=[0.05, ymin+0.07]
     ;;
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[wsource] eq calib_run[irun], nn)
        if nn gt 0 then oplot, exp(-tau_a1[wsource[w]]/sin(elev[wsource[w]])), flux_ratio_a1[wsource[w]], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25), col=col_tab[irun]
        print, 'nscan = ', nn
        bias = mean(flux_ratio_a1[wsource[w]])
        rms  = stddev(flux_ratio_a1[wsource[w]])/mean(flux_ratio_a1[wsource[w]])*100
        print, 'bias = ', bias
        bias_tab[0, irun] =  bias
        print, 'rel.rms = ',rms
        rms_tab[0, irun] = rms
     endfor
     ;;
     legendastro, calib_run, col=col_tab, textcol=0, box=0, charsize=charsize, pos=[0.05, ymin+0.07], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[wsource] eq calib_run[irun], nn)
        if nn gt 0 then oplot, exp(-tau_a3[wsource[w]]/sin(elev[wsource[w]])), flux_ratio_a3[wsource[w]], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25), col=col_tab[irun]
        print, 'nscan = ', nn
        bias = mean(flux_ratio_a3[wsource[w]])
        rms = stddev(flux_ratio_a3[wsource[w]])/mean(flux_ratio_a3[wsource[w]])*100
        print, 'bias = ', bias
        bias_tab[2, irun] =  bias
        print, 'rel.rms = ',rms
        rms_tab[2, irun] = rms
     endfor
     ;;
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[wsource] eq calib_run[irun], nn)
        if nn gt 0 then oplot, exp(-tau_a2[wsource[w]]/sin(elev[wsource[w]])), flux_ratio_a2[wsource[w]], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25), col=col_tab[irun]
        print, 'nscan = ', nn
        bias = mean(flux_ratio_a2[wsource[w]])
        rms  = stddev(flux_ratio_a2[wsource[w]])/mean(flux_ratio_a2[wsource[w]])*100
        print, 'bias = ', bias
        bias_tab[1, irun] =  bias
        print, 'rel.rms = ',rms
        rms_tab[1, irun] = rms
     endfor
     ;;
     legendastro, calib_run, col=col_tab, textcol=0, box=0, charsize=charsize, pos=[0.03, ymax-0.03], psym=cgsymcat('FILLEDCIRCLE', thick=thick*0.25)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     quoi = ['A1', 'A2', 'A3', '1mm']
     calib_run = [calib_run, 'combination']
     ;; SAUVEGARDE FICHIER
