pro script_180_14_rcas, nscans=nscans, list_results_only=list_results_only, keep_current_result=keep_current_result, $
subtract_maps=subtract_maps, preproc=preproc, average=average, reset=reset
if keyword_set(average) then begin
rfile = !nika.plot_dir+'/running_180_14_rcas.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('RCAS') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing RCAS was found in '+db_file
   goto, exit
endif
