pro script_230_14_deepfield1, nscans=nscans, list_results_only=list_results_only, keep_current_result=keep_current_result, $
subtract_maps=subtract_maps, preproc=preproc, average=average, reset=reset
if keyword_set(average) then begin
rfile = !nika.plot_dir+'/running_230_14_deepfield1.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('DEEPFIELD1') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing DEEPFIELD1 was found in '+db_file
   goto, exit
endif
...
param.source        = 'DEEPFIELD1'
param.silent            = 1
param.map_reso          = 2.d0 ; arcsec
param.name4file = 'DEEPFIELD1'
param.project_dir = !nika.plot_dir+'/230_14/'+param.source
...
param.decor_method    = 'common_mode_one_block' ; 'common_mode'
