pro script_222_14_macs0717, nscans=nscans, list_results_only=list_results_only, keep_current_result=keep_current_result, $
subtract_maps=subtract_maps, preproc=preproc, average=average, reset=reset
if keyword_set(average) then begin
rfile = !nika.plot_dir+'/running_222_14_macs0717.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('MACS0717') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing MACS0717 was found in '+db_file
   goto, exit
endif
...
param.source        = 'MACS0717'
param.silent            = 1
param.map_reso          = 2.d0 ; arcsec
param.name4file = 'MACS0717'
