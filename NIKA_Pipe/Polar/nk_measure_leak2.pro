...
; Comments to come, nsubm= size of sub map where the coefficients are
; computed.
; Fit with a nominal Gaussian smoothing
pro nk_measure_leak2, grid, fwhmarr, nsubm, j1, j2, leakcoeff, $
                      verbose = verb, k_3rd = k_3rd
  ...
  if keyword_set( k_3rd) then ncoeff = 10 else ncoeff = 6
  leakcoeff = replicate( {coeff : fltarr(2, ncoeff), $ ; 6 coefficients
                          coeffun : fltarr(2, ncoeff), $ ; uncertainty
  ...
  nk_grid2grad2, grid, fwhmarr, grad, /norm, k_3rd = k_3rd
  ...
  for narr = 0, nuse-1 do begin ; A1 and A3 or all
     ...
     kgauss = get_gaussian_kernel( fwhm[ narr], reso, /nonorm)

     index = lindgen( nmap, nmap)
     subind = index[j1:j2, j1:j2]
     nsub = n_elements( subind)

     xx = dblarr( ncoeff, nsub)
     ...
     if keyword_set( k_3rd) then begin
        xx[6, *] = reform( grad[ indmap].mapxxx[j1:j2, j1:j2], nsub)
        xx[7, *] = reform( grad[ indmap].mapxxy[j1:j2, j1:j2], nsub)
        xx[8, *] = reform( grad[ indmap].mapxyy[j1:j2, j1:j2], nsub)
        xx[9, *] = reform( grad[ indmap].mapyyy[j1:j2, j1:j2], nsub)
     endif
     
     maxmap = max( grid.(maptag[ narr]))
     for iqu = 0, 1 do begin    ; loop on Q and U leakage
        stokes_convol = convol( grid.(qutag[iqu+narr*2]), kgauss)
        yy = dblarr( nsub)
        yy[*] = reform( stokes_convol[j1:j2, j1:j2]/maxmap, nsub)
        ...
        if keyword_set( verb) then begin 
           ...
           print, reform(result), sigma, cc, format = '(11F10.5)'
           print, 'min, max, stddev, Before After'
           print, [minmax(yy), stddev( yy)], format = '(3F10.5)'
           print, [minmax( yy-yyfit), stddev( yy-yyfit)], format = '(3F10.5)'
        endif
