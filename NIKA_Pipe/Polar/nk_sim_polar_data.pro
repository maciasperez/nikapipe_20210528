...
pro nk_sim_polar_data, param, data, kidpar, info1, ps, test_toi, $
...
nk_default_param, param        ;Take the analysis param
nk_init_info, param, info1, /polar

param.output_dir = ps.gen.output_dir
param.scan_num   = ps.gen.scan_num
param.day        = ps.gen.day
...
;;=======================================================================
ps.instr.fwhm_2mm = !nika.fwhm_nom[1]            ; arcsec
ps.instr.fwhm_1mm = !nika.fwhm_nom[0]            ; arcsec

ps.hwp.nu_rot      = 2.38d0               ; hwp rotation frequency [Hz]
...
nk_getdata, param, info1, data0, kidpar
...
param1 = param
param1.map_reso = ps.sky.reso_map
;; nika_pipe_xymaps, param1, data0, kidpar, xmap, ymap, nx, ny, xmin, ymin
ps.sky.nx   = info1.nx
ps.sky.ny   = info1.ny
ps.sky.xmin = info1.xmin
ps.sky.ymin = info1.ymin
nk_simu_polar_maps, ps, maps_S0, maps_S1, maps_S2
;; plot section
;; xmin = min(xmap)-param.map_reso/2.
;; ymin = min(ymap)-param.map_reso/2.
...
;; imview, map_i_1mm, xmap=xmap, ymap=ymap
;; imview, map_q_1mm, xmap=xmap, ymap=ymap
;; imview, map_u_1mm, xmap=xmap, ymap=ymap
;; imview, map_i_2mm, xmap=xmap, ymap=ymap
;; imview, map_q_2mm, xmap=xmap, ymap=ymap
;; imview, map_u_2mm, xmap=xmap, ymap=ymap
;; ******************************************
stop
...
if n_sources ne 0 then begin
   ps_ra  = randomu( seed, n_sources)*(max(info1.xmap)-min(info1.xmap)) + info1.xmin
   ps_dec = randomu( seed, n_sources)*(max(info1.ymap)-min(info1.ymap)) + info1.ymin
...
junk = data0[0]
;; upgrade_struct, junk, {position:0.d0}, junk
...
data.position   = (2.d0*!dpi*ps.hwp.nu_rot*time) mod (2.d0*!dpi) ; HWP rotation angle
...
if ps.gen.add_template ne 0 then $
   make_template, ps.template.n_harmonics, data.position*!radeg, time, $
...
for lambda=1, 2 do begin
   ...
   if nwlambda ne 0 then begin
      for i=0, nwlambda-1 do begin
         ...
         ix   = long( (dra  - info1.xmin)/ps.sky.reso_map)   ; Coord of the pixel along x
         iy   = long( (ddec - info1.ymin)/ps.sky.reso_map)   ; Coord of the pixel along y
         ...
         if i eq 0 then begin
            apply_mueller, stokes, data.position, hwp_mueller, junk
         ...
         ;;--------------------------------------------------------------
         ;; HWP modulation
         apply_mueller, stokes, data.position, hwp_mueller, stokes
         ;;--------------------------------------------------------------
         ...
         data.toi[ikid] = stokes[*,0]

         ;; Add template (same of all kids for now)
         if ps.gen.add_template ne 0 then data.toi[ikid] += hwp_template
...
if n_sources ne 0 then begin
   for is=0, n_sources-1 do begin
      d     = sqrt( (info1.xmap-ps_ra[is])^2+(info1.ymap-ps_dec[is])^2)
