...
pro nk_latex_maps_report, param, scan_list, project_dir=project_dir, plot_dir=plot_dir

if n_params() lt 1 then begin
   message, /info, "Calling sequence:"
   print, "nk_latex_maps_report, param, scan_list"
   return
endif
...
filin = project_dir+'/maps_report_'+strtrim(str_replace(param.source," ", "_"),2) +$
        '_'+strtrim( scan_list[0], 2)+ suffix+ '.tex'
;;;filin = param.project_dir+"/project_report_"+strtrim(param.source,2)+".tex"
...
;; printf, tex_unit, "{\large \bf "+strupcase(source_tex_name)+", "+strmid( systime(0),0,10)+"}\\"
;; printf, tex_unit, "\end{center}"

;;--------------------------------------------------------------------------
;; Average maps on cover page
ext1 = "ps"
init_figure = 0
stokes = ['I', 'Q', 'U']
for lambda=1,2 do begin
   for istokes=0, 2 do begin
      file_1 = plot_dir+"/maps_"+strtrim( str_replace(param.source," ", "_"),2)+$
               "_"+strtrim(scan_list[0],2)+"_nsc"+strtrim(n_elements(scan_list),2)+"_"+$
               stokes[istokes]+"_"+strtrim(lambda,2)+"MM.ps"
      if file_test(file_1) then begin
         if init_figure eq 0 then begin
            printf, tex_unit, "\begin{figure}[hhh]"
            printf, tex_unit, "\begin{center}"
            init_figure++
         endif
         printf, tex_unit, "\includegraphics[clip, angle=0, scale = 0.4]{"+file_1+"}"
      endif
   endfor
endfor
if init_figure ne 0 then begin
   printf, tex_unit, "\caption{Average maps.}"
   printf, tex_unit, "\end{center}"
...
for iscan=0, nscans-1 do begin
   ...
   for iarray=1, 3 do begin
      for istokes=0, 2 do begin
         map_file = plot_dir+"/maps_"+strtrim(scan_list[iscan],2)+"_"+stokes[istokes]+strtrim(iarray,2)+".ps"
         if file_test(map_file) then begin
            if init_figure eq 0 then begin
            ...
            printf, tex_unit, "\includegraphics[clip, angle=0, scale = "+strtrim(scale,2)+"]{"+map_file+"}"
         endif
      endfor
   endfor
   if init_figure ne 0 then begin
...
if param.latex_pdf eq 1 then begin
   ...
   spawn, "latex -halt-on-error "+filin+ext
   dvi_file = "maps_report_"+strtrim(str_replace(param.source," ", "_"),2)+$
              '_'+strtrim( scan_list[0], 2)+ suffix+".dvi"
   if file_test(dvi_file) then spawn, "dvipdf "+dvi_file+ext
   spawn, "\mv maps_report_"+strtrim(str_replace(param.source," ", "_"),2)+$
