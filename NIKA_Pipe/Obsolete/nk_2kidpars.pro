...
pro nk_2kidpars, scan_list_in, kidpar_4projection_file, percentile=percentile, param=param, info=info, $
        filing=filing, data=data, kidpar=kidpar, $
        print_status=print_status, grid=grid, $
        simpar=simpar, sn_min_list=sn_min_list, sn_max_list=sn_max_list, $
        subtract_maps=subtract_maps, no_output_map=no_output_map, prism=prism, $
...
for iscan=0, nscans-1 do begin
   ...
   if process_file ne 0 then begin
      ...
      nk_scan_preproc, param, info, data, kidpar, grid, preproc_copy=param.preproc_copy, $
      ...
      ;;----------------------------------------------------------------
      ;; LP: modification to project a fraction of the KID only
      if keyword_set(percentile) then begin
         select_best_kid_percentile, kidpar, output_kidpar, percentile, out_nefd_thres, /nofile, /double_cut
         kidpar = output_kidpar
      endif else begin
         kidpar_toproject = mrdfits(kidpar_4projection_file, 1)
         w1 = where( kidpar_toproject.type eq 1, nw1)
         w  = where( kidpar.type eq 1, nw)
         kidpar_toproject = kidpar_toproject[w1]
         kidpar  = kidpar[w]
         my_match, kidpar.numdet, kidpar_toproject.numdet, suba, subb
         kidpar.type = 0
         kidpar[suba].type = kidpar_toproject[subb].type
      endelse
      ;;-----------------------------------------------------------------
      ...
      nk_save_scan_results_3, param, info, data, kidpar, grid, filing=filing, xguess=xguess, yguess=yguess
...
;      stop
   ...
   ; NP helps FXD to build maps per subscan NOT DONE YET
;;    if param.map_per_subscan eq 1 then begin
...
;;             save, grid, ..., file='map_of_subscan_'+strtrim(isub,2)+".save"
;;          endif
;;       endfor
