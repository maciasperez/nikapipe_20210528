...
pro nk_skydip_2, scan_num, day, kidpar, param=param, sav=sav, help=help, test=test, png=png, ps=ps, RF=RF, $
                 no_acq_flag=no_acq_flag, input_kidpar = input_kidpar
...
if keyword_set(input_kidpar) then kidpar = input_kidpar
...
;/////////////////////////////////////////////////////////
   ...
   subscan_f=where(shifted eq 5) ;cambiare
   backot=where(shifted eq 6)
if n_elements(backot) gt n_elements(subscan_s) then  backot = backot[1:n_elements(backot)-1]
if n_elements(subscan_f) gt n_elements(subscan_s) then subscan_f =  subscan_f[1:n_elements(subscan_f)-1]
backot = backot [1:n_elements(backot)-1]   ; CAMBIARE se si vuole togliere un punto
subscan_f = subscan_f[1:n_elements(subscan_f)-1]; CAMBIARE se si vuole togliere un punto 
subscan_s =  subscan_s[1:n_elements(subscan_s)-1]; CAMBIARE se si vuole togliere un punto
...
;///////////                                                                   
      ...
      air_mass=1./sin(ell)
;
;ffrx = ffrx[2:n_elements(ffrx)-1]
;air_mass =  air_mass[2:n_elements(air_mass)-1]
;                                                                                
      taup=0.
      tatm=270.
      if channel eq 1 then p_start=[1600,0.1,-2.0d9] ;,1.d5]     
      if channel eq 2 then p_start=[800,0.08,-1.4d9] ;,1.d5]
...
;      estimate=[1790.5466,0.050000001,-1.8193226e+09]
      if channel eq 1 then parinfo[0].limits=[1000.,2000.] ;CAMBIATO DA 2500 a 4500!!!
      if channel eq 2 then parinfo[0].limits=[300.,1500.];CAMBIATO DA 1500 a 3500!!! 
      parinfo[1].limits=[0.0,0.8]
      parinfo[2].limits=[-1.d10,-1.d8]  
; CAMBIARE il numero di punti da utilizzare                                                  
;      if channel eq 1 then      air_mass = air_mass[1:n_elements(air_mass)-1]
;      if channel eq 1 then      ffrx = ffrx[1:n_elements(ffrx)-1]
;
      fito = mpfitfun("tau_model2", air_mass, ffrx, e_r,p_start, /quiet, parinfo=parinfo)
;
;  selection kids not good                                                                                
      fittt =  fito(2)+fito(0)*270.d0*(1.d0-exp(-air_mass*fito(1)))
      if channel eq 1 then bad = where(abs(1.d5*(ffrx - fittt ) / fittt) gt 10)
      if channel eq 2 then bad =  where(abs(1.d5*(ffrx - fittt ) / fittt) gt 10)
      if bad(0) gt -1 then fito(*) = 0.
;
      tau(j)=fito(1)
      K(j)=fito(0)
      fto(j)=fito(2)            ;      
;  selection kids not good
;
      check(j) = 1.d7*mean(abs((ffrx - fittt) /fittt))
      if test eq 1 then begin
...
;         check(j)=1.d-7*(abs(ffrx - fittt) / (fittt))
;         oplot,  air_mass,  1.d5*(ffrx - fittt ) / (fittt), color = 250
         wait,0.1
      endif
   endfor
;
;stop                                                  
; selection on kid based on bad fit and tau variation
;   idx1 = where(sqrt((tau-mean(tau))^2) lt 0.7*stddev(tau))   
   idx1=where(tau gt 0)
   idx2 = where(sqrt((tau(idx1)-mean(tau(idx1)))^2) lt 0.7*stddev(tau(idx1)))
;
   tau_v=tau(idx1(idx2))
   Const=K(idx1(idx2))
   f_to=fto(idx1(idx2))
...
;   !P.multi=[0,1,2]
;stop
   FIT_func=dblarr(n_elements(idx2),n_elements(ffrx))
   for i=0,n_elements(idx2)-1 do FIT_func(i,*)=f_to(i)+Const(i)*Tatm*((1.-exp(-1.*air_mass(*)*tau_v(i))))
;
;Save intermediate results**************************************************************
   ss=string(scan_num)
   ss=strtrim(ss,2)
   if keyword_set(sav) then save,filename='skydip'+day+ss+tit+'.save',air_mass,ft,dft,idx1(idx2),fit_func
;***************************************************************************************                  
...
;   fit_func=dblarr(n_elements(const),n_elements(air_mass))
   for j=0,n_elements(const)-1 do begin
      ...
      if channel eq 1 then begin 
         ...
         kidpar(w_on1(flagkid(idx1(idx2(j))))).c0_skydip=fi(j)
         kidpar(w_on1(flagkid(idx1(idx2(j))))).c1_skydip=ki(j)
      endif
      if channel eq 2 then begin 
         if ki(j) lt 10. then ki(j)=0
         kidpar(w_on2(flagkid(idx1(idx2(j))))).c0_skydip=fi(j)
         kidpar(w_on2(flagkid(idx1(idx2(j))))).c1_skydip=ki(j)
      endif
;      fit_func(j,*)=fi(j)+ki(j)*Tatm*((1.-exp(-1.*air_mass(*)*tau_fix)))
   endfor
;                                                                                  
   if channel eq 1 then  begin 
      skydip_final1={channel:tit ,scan_number:scan_num , day:day , valid_kid:flagkid(idx1(idx2)) , par1:fi , par2:ki , par3:tau_fix}
      skydip_res1mm={fit1mm:fit_func, am1mm:air_mass , ft1mm:ft , dft1mm:dft ,idx1mm:idx2 , tau1mm:tau_v, qual1mm:check}
   endif
; 
   if channel eq 2 then begin
      skydip_final2={channel:tit ,scan_number:scan_num , day:day , valid_kid:flagkid(idx1(idx2)) , par1:fi , par2:ki , par3:tau_fix}
      skydip_res2mm={fit2mm:fit_func, am2mm:air_mass , ft2mm:ft , dft2mm:dft ,idx2mm:idx2 , tau2mm:tau_v,  qual2mm:check}
   endif
;stop
endfor                                                                                 
;stop
...
if test eq 1 then begin
!P.multi = [0, 1, 1]
window, 14, title = ' SKYDIP FIT QUALITY', xsize = 1200, ysize = 400
n_histwork,  skydip_res1mm.qual1mm, bin = 0.4, /fill, fcolor = 70,xr=[0,10],yr=[0,40], ytitle = '%x10^-7'
n_histwork,   skydip_res2mm.qual2mm,  bin =  0.4, /fill, fcolor = 250, /overplot
endif
;
!P.multi=[0,1,2]
;                                                                                  
