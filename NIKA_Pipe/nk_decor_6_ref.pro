...
;;          endif
   ...
   nsn = n_elements(data)
   if param.decor_period le 0 then work_on_entire_scan = 1 else work_on_entire_scan = 0

   ;; special case
   if param.decor_period gt (nsn*!nika.f_sampling) then begin
      message, /info, "the requested param.decor_period "+strtrim(param.decor_period,2)+" seconds"
      message, /info, "is larger than the duration of the scan. I'll decorrelate on the full scan then."
      work_on_entire_scan = 1
   endif

   if work_on_entire_scan eq 1 then begin
      case istokes of
         1: toi = data.toi
         2: toi = data.toi_q
         3: toi = data.toi_u
      ...
      ;; work on sub-parts of the scan sequentially
      i1 = 0
      i2 = 0
      n = round( param.decor_period*!nika.f_sampling) < (nsn-1)
      pp = 1

      while i2 lt (nsn-1) do begin
         i2 = (i1 + n) < (nsn-1)
         i1 = i2-n
         print, "i1, i2, nsn: ", i1, i2, nsn
         if param.log then nk_log, info, "decorr on samples "+$
                                   strtrim(i1,2)+" to "+strtrim(i2,2)+$
                                   " according to param.decor_period="+strtrim(param.decor_period,2)
         ...
         case istokes of
            1: data[i1:i2].toi   = toi 
            2: data[i1:i2].toi_q = toi
            3: data[i1:i2].toi_u = toi
         endcase
         data[i1:i2].flag         = flag
         out_temp_data[i1:i2].toi = out_temp.toi
         if param.subscan_edge_w8 gt 0 then hfnoise_w8[*,i1:i2] = w8_hfnoise
