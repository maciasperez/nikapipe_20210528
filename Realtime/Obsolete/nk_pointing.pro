...
offsets = dblarr(2,2)
nterms = 5
plot_file = plot_output_dir+"/plot_"+strtrim(param.scan)
if param.plot_png eq 1 then message, /info, "Outplot in: "+plot_file+".png"
if param.plot_ps  eq 1 then message, /info, "Outplot in: "+plot_file+".ps"
outplot, file=plot_file, png = param.plot_png, ps = param.plot_ps

fwhm_prof = dblarr(2,2,2)       ; 2 arrays, (az,el), forward/backward scan
ellipticity = dblarr(2)
display_info = 0
if strupcase( strtrim( pako_str.obs_type, 2)) eq "POINTING" then begin
   wind, 1, 1, /free, xs=1500, ys=700, title = 'nk_pointing', iconic = param.iconic
   ...
   for lambda=1, 2 do begin
      ...
      if nw1 ne 0 then begin
         ...
         for i=0, 1 do begin
            ...
            fwhm_prof[lambda-1,i,0] = a1[2]/!fwhm2sigma
            ...
            if display_info eq 0 then begin
               legendastro, ['Focusz: '+string(info.focusz, format = "(F6.2)"), $
                             ...
                             "El: "+string(avg(data.el)*!radeg, format = "(F6.2)")], box = 0, /right, /bottom
               display_info = 1
            endif
            ...
            fwhm_prof[lambda-1,i,1] = a2[2]/!fwhm2sigma
            print, "lambda : "+strtrim(lambda,2)
            print, "fwhm_prof "+xtitle+": ", strtrim( reform(fwhm_prof[lambda-1,i,*]),2)
...
;            print, ";; focus x="+string(info.focusx,  format =  "(F5.2)")+$
;                   ", focus y ="+string(info.focusy, format="(F5.2)")+$
;                   ", focus z ="+string(info.focusz, format="(F5.2)")
;            print, ";; lambda = "+strtrim(lambda,2)+", fwhm = "+num2string(a1[2]/!fwhm2sigma)
   ...
   my_multiplot, 3, 2, /rev, pp, pp1, xmin = 0.15, xmax = 0.9, gap_x = 0.05
   wind, 1, 1, /free, /xlarge, title = 'nk_pointing', iconic = param.iconic
...
;         map_var = info.map_var_2mm
      ...
      ellipticity[lambda-1] = output_fit_par[2]/output_fit_par[3]
      print, "lambda: "+strtrim(lambda,2)+", ellipt: "+strtrim(ellipticity[lambda-1],2)
   endif
ciao:
endfor
outplot, /close
...
log_info.result_name[ 0] = 'az offset'
log_info.result_value[0] = ptg_az_instruction
log_info.result_name[ 1] = 'el offset'
