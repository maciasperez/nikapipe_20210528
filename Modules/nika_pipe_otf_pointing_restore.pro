pro nika_pipe_otf_pointing_restore, param, data, kidpar, plot=plot, chatty=chatty

;; OTF pointing restoration 
;; In case of remaining holes after nika_pipe_antenna2pointing_2
;; (i. e. OTF scan with holes in the imbfits), check the
;; interpolation and restore the pointing if enough valid
;; information (hole size < 2 subscans)
;;
;; contact: perotto@lpsc.in2p3.fr
;;
 
  code = "NIKA_PIPE_OTF_POINTING_RESTORE >> "
  bav = 0
  ...
  if strtrim( strupcase( obs_type),2) eq "ONTHEFLYMAP" then begin
     ...
     ;; flag out holes 
     ;; antenna2pointing flagged missing samples in the imbfits pointing by adding
     ;; 2L^18 
     w1         = where( kidpar.type eq 1, nw1)
     flag       = data.flag[w1[0]]
     w_holes_w  = nika_pipe_wflag( flag[w], 18, nflag=nflag)
     ...
     if nflag gt 0 then begin
        ...
        junk = mrdfits( param.imb_fits_file,2,hdr,/sil)
        projection = sxpar( hdr, 'SYSTEMOF',/silent)
        if bav gt 0 then print,code, "projection type is ", projection         
        scan_type = 'type'
        otf_pointing_azel2xy, data.ofs_az, data.ofs_el, flag, projection, x, y, angle, scan_type=scan_type
        if bav gt 0 then print,code, "type of OTF scan is ", scan_type
                
        ;; MAIN ROUTINE PERFORMING THE RESTORATION
        otf_pointing_restore, x, y, flag, x_restored, y_restored, outflag_restored, ind_beg_scan=ibeg, ind_end_scan=iend, chatty=chatty, debug=debug
        ...
        if (n0 gt 0) then begin
           ...
           if (n_no_miss_ext gt 0) then begin
              ...
              data[w0[w_no_miss_ext]].flag -= 2L^18
        ...
        if (n_restored gt 0) then begin
           ...
           if (n_bad gt 0) then begin
              ...
              ;; for plotting purpose only
              xrmin2 = index[w_safe[0]]
              xrmax2 = index[w_safe[n_safe-1]]
           endif
        endif
        ...
        otf_pointing_xy2azel, x, y, angle, az, el
        data.ofs_az = az
        data.ofs_el = el
        az=0B
        el=0B
     ...
     endif else print,code," no holes....I do nothing."
     
  endif else print,code," not an OTF scan....I do nothing."
