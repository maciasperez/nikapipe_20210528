...
pro nika_pipe_map_2, param, data, kidpar, maps, $
  ...
  if n_params() lt 1 then begin
     ...
     print, "nika_pipe_map_2, param, data, kidpar, maps, $"
  ...
  if not keyword_set(astr) then begin
     reso_x = param.map_reso
     reso_y = param.map_reso
     ...
     xymaps, nx, ny, xmin, ymin, param.map_reso, xmap, ymap, xgrid, ygrid
     c_arcsec_x = 0.0
     c_arcsec_y = 0.0
  endif
     ...
     if kidpar[ikid].type eq 1 then begin
        ...
        if keyword_set(azel) then begin
           ...
           if keyword_set(nasmyth) then begin
              ...
              dra  = ofs_x - kidpar[ikid].nas_x
              ddec = ofs_y - kidpar[ikid].nas_y
        ...
        for isn=0l, N_pt-1 do begin
           ...
           if (ix[isn] ge 0) and (ix[isn] le (nx-1)) and $
              ...
              if kidpar[ikid].array eq 1 and (not keyword_set(two_mm_only)) then begin
                 map_A[   p] += data[isn].w8[ikid]*data[isn].toi[ikid]
              ...
              if kidpar[ikid].array eq 2 and (not keyword_set(one_mm_only)) then begin
                 map_B[   p] += data[isn].w8[ikid]*data[isn].toi[ikid]
...
;;-------------- Moyenne sur le nombre de coups
  ...
  if not keyword_set(bypass_error) then if not keyword_set(two_mm_only) and nwpos_var_A eq 0 then message, "No pixel with more than 2 hits at 240GHz/1mm ?!"
  if not keyword_set(bypass_error) then if not keyword_set(one_mm_only) and nwpos_var_B eq 0 then message, "No pixel with more than 2 hits at 140GHz/2mm ?!"

  if nwpos_A ne 0 then map_A[pos_A] /= map_w8_A[pos_A]
  if nwpos_B ne 0 then map_B[pos_B] /= map_w8_B[pos_B]

  if not keyword_set(bypass_error) then if not keyword_set(two_mm_only) and nwpos_A eq 0 then message, "No pixel was hit at 240GHz ?!"
  if not keyword_set(bypass_error) then if not keyword_set(one_mm_only) and nwpos_B eq 0 then message, "No pixel was hit at 140 GHz?!"
     ...
     for i=0, n_on-1 do begin
        ...
        if keyword_set(azel) then begin
           ...
           if keyword_set(nasmyth) then begin
              ...
              dra  = ofs_x + kidpar[w_on[ikid]].nas_x
              ddec = ofs_y + kidpar[w_on[ikid]].nas_y
           endif else begin     ; then do radec     
              
              nika_nasmyth2draddec, data.ofs_az, data.ofs_el, data.el, data.paral, $
                                    kidpar[w_on[ikid]].nas_x, kidpar[w_on[ikid]].nas_y, $
                                    0., 0., dra, ddec, nas_x_ref=kidpar[w_on[ikid]].nas_center_X, $
        ...
        for isn=0l, N_pt-1 do begin
           ...
           if (ix[isn] ge 0) and (ix[isn] le (nx-1)) and (iy[isn] ge 0) and (iy[isn] le (ny-1)) and $
              ...
              map_per_scan_per_kid[param.iscan,ikid].Jy[p] += data[isn].w8[w_on[ikid]]*data[isn].toi[w_on[ikid]]
