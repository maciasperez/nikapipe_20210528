...
#Change by Florian:
#-----------------
hdu = fits.open('/Users/ruppin/Projects/NIKA/Soft/branch/Florian/FR/These/Neural_network/Data_beammap/image_dataset_fin_5.fits')
data = hdu[0].data
data = np.transpose(data)
hdu2 = fits.open('/Users/ruppin/Projects/NIKA/Soft/branch/Florian/FR/These/Neural_network/Data_beammap/class_dataset_fin_5.fits')
targets = hdu2[0].data
data = data[:, np.newaxis, :, :]
(trainData, testData, trainLabels, testLabels) = train_test_split(
	data, targets.astype("int"), test_size=0.1)
#==================
#data = data[:, np.newaxis, :, :]
#(trainData, testData, trainLabels, testLabels) = train_test_split(
...
trainLabels = np_utils.to_categorical(trainLabels, num_classes=5)
testLabels = np_utils.to_categorical(testLabels, num_classes=5)

# initialize the optimizer and model
print("[INFO] compiling model...")
opt = SGD(lr=0.01)
model = LeNet.build(width=221, height=221, depth=1, classes=5,
...
if args["load_model"] < 0:
	print("[INFO] training...")
	model.fit(trainData, trainLabels, batch_size=128, epochs=1000,
		verbose=1)
