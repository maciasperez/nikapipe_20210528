...
;
; NAME: 
; nk_measure_beam_volume
;
; CALLING SEQUENCE:
;         f = nk_measure_beam_volume( lambda_mm, file, int_rad=int_rad, no_file=no_file)
; 
; PURPOSE: 
;        Comput the volume covered by the beam up to a given distance
; 
; INPUT: 
;       - lambda_mm: the wavelenght in mm
;       - file: the beam fits file (string)
; 
; OUTPUT: 
;        - The beam volume in arcsec^2
; 
; KEYWORDS:
;         -  int_rad: the radius at which the integration stops
;            (default is 1 arcmin)
...
function nk_measure_beam_volume, lambda_mm, file, int_rad=int_rad, no_file=no_file
