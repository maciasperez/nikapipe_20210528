...
pro nk_skydip_test, scan, param=param, sav=sav, help=help, xml=xml, $
...
;; simpler and faster
param.math = "RF" ; to skip the computation of PF
...
if tag_exist( data, "C_SYNCHRO") then info.polar = 1
...
wind, 1, 1, /free, xs=1200, ys=900, $
      title='nk_skydip '+strtrim(param.scan,2), iconic = param.iconic
      ...
      for j=0,nw1-1 do begin
         ...
         for i=0,nsubscans-1 do begin
            ...
            w = where( data.flag[ikid] eq 0 and data.subscan eq valid_subscan[i] and $
                       time ge time[backot[i]] and time le tmax and $
...
!P.multi=[0,1,2]
wind, 1, 1, /free, title=' SKYDIP FIT Results',xsize=1200,ysize=400, iconic = param.iconic
