...
pro nk_decor_6, param, info, data, kidpar, grid, out_temp_data, $
                out_coeffs=out_coeffs, $
                snr_toi=snr_toi, Q=Q, U=U, $
...
;;    message, /info, "fix me: restrict to 3 subscans"
;;    w = where( data.subscan ge 2 and data.subscan le 4)
;;    data = data[w]
;;    stop
;; endif
...
;; message, /info, "HERE:"
;; w = where( data.subscan eq 38)
;; print, minmax(finite(data[w].toi[1295]))
;; stop
...
;;          endif
         ...
         info.CURRENT_SUBSCAN_NUM = long(isubscan)
         ...
         nk_decor_sub_6, param, info, toi, flag, off_source, kidpar, $
                         ...
                         dra=data[wsample].dra, ddec=data[wsample].ddec, $
                         zm_toi=zm_toi[*,wsample], $
...
endif else begin ; decor per scan
   ...
   if param.tiling_decorrelation then begin
      ...
      for isubscan = isubmin, isubmax do begin
         ...
         if param.trigo_modes_period_arcsec gt 0. then begin
            ;; Factor 2 because in this version of tiling, the time
            ;; scale is on two subscans
            param.nharm_multi_sinfit = round( 2*info.subscan_arcsec/param.trigo_modes_period_arcsec)
         endif
         ...
         if keyword_set(snr_toi) then my_snr_toi = snr_toi[*,i1:i2] else delvarx, my_snr_toi
         nk_decor_sub_6, param, info, toi, flag, off_source, kidpar, $
      ...
      endcase
      case istokes of
         1: subtoi = datasub.toi
         2: subtoi = datasub.toi_q
         3: subtoi = datasub.toi_u
      endcase
      flag = data.flag
      elevation = data.el
      ...
      if param.trigo_modes_period_arcsec gt 0. then begin
         param.nharm_multi_sinfit = $
            round( nsubscans*info.subscan_arcsec/ $
                   param.trigo_modes_period_arcsec)
      endif
      ...
      nk_decor_sub_6, param, info, toi, flag, data.off_source, kidpar, $
                      ...
                      out_coeffs=out_coeffs, snr_toi=snr_toi, $
                      subscan=data.subscan, $
                      dra=data.dra, ddec=data.ddec, zm_toi=zm_toi, $
