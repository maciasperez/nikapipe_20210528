...
;        - Nov. 26th, 2017: A. Ritacco - adapted to NIKA data
;-

pro nk_lkg_correct_temp_nika, param, info, data, kidpar, grid, $
                              input_polar_maps, lkg_kernel, input_polar_maps_param, lkg_kernel_param, $
                              align=align, gauss_regul=gauss_regul, astr=astr
...
nk_maps2data_toi_nika, param, info, data, kidpar, $
...
if keyword_set(align) then begin
   ...
   nk_map_photometry, grid_in_nas_temp.map_i2, grid_in_nas_temp.map_var_i2, grid_in_nas_temp.nhits_i2, $
                      grid_in_nas_temp.xmap, grid_in_nas_temp.ymap, !nika.fwhm_nom[0], $
                      flux, sigma_flux, sigma_bg, output_fit_par_input, /educated, /noplot
   nk_map_photometry, grid_data_nasmyth.map_i2, grid_data_nasmyth.map_var_i2, grid_data_nasmyth.nhits_i2, $
...
nk_get_ipix, param1, info, data_copy_lkg, kidpar, grid_input_nasmyth, astr=astr
...
;; Init wave vectors
give_map_k, lkg_kernel.map_reso, lkg_kernel.map_i2*0.d0, map_k
...
for lambda=1, 2 do begin
   ...
   if lambda eq 1 then begin
      map_i = grid_input_nasmyth.map_i1
      message, /info, "fix me: kmax is still hardcoded"
      kmax = 0.2
      i_kernel = lkg_kernel.map_i1
      q_kernel = lkg_kernel.map_q1
      u_kernel = lkg_kernel.map_u1
   endif else begin
      map_i = grid_input_nasmyth.map_i2
      message, /info, "fix me: kmax is still hardcoded"
      kmax = 0.08
      i_kernel = lkg_kernel.map_i2
      q_kernel = lkg_kernel.map_q2
      u_kernel = lkg_kernel.map_u2
   endelse
...
;      return
      ...
      gu = gq
      give_map_k, 1, lkg_kernel.map_i1, map_k

      fft_i  = fft( lkg_kernel.map_i1, /double)
      fft_ig = fft( lkg_kernel.map_i1*gi, /double)
      fft_qg = fft( lkg_kernel.map_q1*gq, /double)
      fft_ug = fft( lkg_kernel.map_u1*gu, /double)
   ...
   if param.do_plot ne 0 then begin
      if lambda eq 1 then begin
         ...
         imview, grid_input_nasmyth.map_q1, title='Q radec2nasmyth 1mm', position=pp1[0,*], imr=imr_q, /nobar
         imview, grid_input_nasmyth.map_u1, title='U radec2nasmyth 1mm', position=pp1[1,*], imr=imr_u, /nobar, /noerase
         imview, grid_input_nasmyth.map_q2, title='Q radec2nasmyth 2mm', position=pp1[2,*], imr=imr_q, /nobar, /noerase
         imview, grid_input_nasmyth.map_u2, title='U radec2nasmyth 2mm', position=pp1[3,*], imr=imr_u, /nobar, /noerase
