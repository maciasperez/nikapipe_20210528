...
pro nk_map2fits_ar, param, info, grid, suffix=suffix, $
                 output_file_1mm=output_file_1mm, $
                 output_file_2mm=output_file_2mm
  
if n_params() lt 1 then begin
   message, /info, "Calling sequence:"
   print, "nk_map2fits_ar, param, info,grid"
   return
endif
...
output_info_1mm= {N_scan:info.nscan, $
                  total_obs_time:info.total_obs_time, $
                  tau_240GHz_avg:info.result_tau_1mm, $
                  tau_140GHz_avg:info.result_tau_2mm, $
                  az:info.az_source*!radeg, $
                  el:info.el_source*!radeg, $
                  scan_type:info.obs_type, $
                  fwhm_240Ghz:12.5}

output_info_2mm= {N_scan:info.nscan, $
                  total_obs_time:info.total_obs_time, $
                  tau_240GHz_avg:info.result_tau_1mm, $
                  tau_140GHz_avg:info.result_tau_2mm, $
                  az:info.az_source*!radeg, $
...
if tag_exist( grid, 'map_i1') then begin
   file = strtrim(param.output_dir,2)+'/MAPS_1_'+strtrim(param.name4file,2)+'_v'+strtrim(param.version,2)+ext
   output_file_1mm = file
   mwrfits, 0, file, primaryHeader, /CREATE ;; Create an primaryHDU, could have lots and lots of header

   IF tag_exist(grid, 'map_q1') OR tag_exist(grid, 'map_u1') THEN BEGIN
      ;; We have a polar map
      map = [ [[grid.map_i1]], [[grid.map_i1*!VALUES.F_NAN]], [[grid.map_i1*!VALUES.F_NAN]] ]
      std = [ [[sqrt(grid.map_var_i1)]], [[grid.map_i1*!VALUES.F_NAN]], [[grid.map_i1*!VALUES.F_NAN]] ]
      IF tag_exist( grid, 'map_q1') THEN BEGIN
         map[*,*,1] = grid.map_q1
         std[*,*,1] = sqrt(grid.map_var_q1)
      ENDIF
      IF tag_exist( grid, 'map_u1') THEN BEGIN
         map[*,*,2] = grid.map_u1
         std[*,*,2] = sqrt(grid.map_var_u1)
      ENDIF
   ...
   message, /info, "Wrote "+strtrim(file,2)
endif

;; Write the 2mm results if present
if tag_exist( grid, 'map_i2') then begin
   file = strtrim(param.output_dir,2)+'/MAPS_2_'+strtrim(param.name4file,2)+'_v'+strtrim(param.version,2)+ext
   output_file_2mm = file
   mwrfits, 0, file, primaryHeader, /CREATE ;; Create an primaryHDU, could have lots and lots of header
   
   IF tag_exist(grid, 'map_q2') OR tag_exist(grid, 'map_u2') THEN BEGIN
      ;; We have a polar map
      map = [ [[grid.map_i2]], [[grid.map_i2*!VALUES.F_NAN]], [[grid.map_i2*!VALUES.F_NAN]] ]
      std = [ [[sqrt(grid.map_var_i2)]], [[grid.map_i2*!VALUES.F_NAN]], [[grid.map_i2*!VALUES.F_NAN]] ]
      IF tag_exist( grid, 'map_q2') THEN BEGIN
         map[*,*,1] = grid.map_q2
         std[*,*,1] = sqrt(grid.map_var_q2)
      ENDIF
      IF tag_exist( grid, 'map_u2') THEN BEGIN
         map[*,*,2] = grid.map_u2
         std[*,*,2] = sqrt(grid.map_var_u2)
      ENDIF
   ...
   ;; Special case for the hit map, always the same for all stokes ?!?
   mkhdr, head_map, grid.nhits_2,/EXTEND,/IMAGE        ; typical header
   putast, head_map, astrometry, equinox=2000, cd_type=0 ;astrometry in header
   sxaddpar, head_map, 'EXTNAME', 'Time per pixel map'
   sxaddpar, head_map, 'UNIT', 'second'
   
   mwrfits, grid.nhits_2/!nika.f_sampling, file, head_map, /silent
