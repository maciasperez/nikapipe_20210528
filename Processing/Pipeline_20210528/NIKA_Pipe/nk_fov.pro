...
pro nk_fov, scan_list_in, param=param, info=info, $
            filing=filing, data=data, kidpar=kidpar, $
            print_status=print_status, grid=grid, $
            simpar=simpar, sn_min_list=sn_min_list, sn_max_list=sn_max_list, $
            subtract_maps=subtract_maps, no_output_map=no_output_map, prism=prism, $
...
;      stop
      ...
      ;;-------------------
      ;; map on different regions of the FOV
      xlist = [-3.25, -1.6, 0, 1.6, 3.25, $
               -1.6, 0, 1.6, 0, $
               -1.6, 0, 1.6, 0]
      ylist = [    0,    0, 0,   0,    0, $
                   1.6, 1.6, 1.6, 3.2, $
                   -1.6, -1.6, -1.6, -3.2]
      xlist *= 60.              ; arcmin to arcsec
      ylist *= 60.
      nx = n_elements(xlist)
      dmax = 1.2*60.
      for i=0, nx-1 do begin
         kidpar1 = kidpar
         d = sqrt( (kidpar.nas_x-xlist[i])^2 + (kidpar.nas_y-ylist[i])^2)
         w = where( d gt dmax, nw, compl=wkeep, ncompl=nwkeep)
         if nwkeep ne 0 then begin
            kidpar1[w].type = 3 ; reject all kids outside the disk
         endif
         nk_projection_4, param, info, data, kidpar1, grid
         save, param, info, kidpar1, grid, file=!nika.plot_dir+"/grid_"+strtrim(i,2)+".save"
         message,/info, "saved "+!nika.plot_dir+"/grid_"+strtrim(i,2)+".save"
      endfor
      ;;-------------------
