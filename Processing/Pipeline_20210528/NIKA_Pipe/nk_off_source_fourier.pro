...
pro nk_off_source_fourier, param, info, data, kidpar

if n_params() lt 1 then begin
   message, /info, "calling sequence:"
   print, "nk_off_source_fourier, param, info, data, kidpar"
   return
endif
...
if param.debug eq 1 then begin
   ...
   for i=0, nw1-1 do begin
      ikid = w1[i]
      d = sqrt( data.dra[ikid]^2 + data.ddec[ikid]^2)
      data.off_source[ikid] = double( d ge 50)
   endfor
endif
...
if nw eq 0 then begin
   ...
   for i=0, nw1-1 do begin
      ...
      if nw_on ne 0 then begin
         ...
         if param.debug ne 0 then oplot, time, data.off_source[ikid], col=200, thick=2

         ;; junk  = interpol( smooth( data[w_off].toi[ikid], 20,
         ;; /edge_zero), index[w_off], index)
         t_smooth = 4.d0
         nsmooth = round( t_smooth*!nika.f_sampling)
         junk  = interpol( smooth( data[w_off].toi[ikid], nsmooth, /edge_zero), index[w_off], index)
