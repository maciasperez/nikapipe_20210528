...
pro nk, scan_list_in, param=param, info=info, $
        filing=filing, data=data, kidpar=kidpar, $
        print_status=print_status, grid=grid, $
        simpar=simpar, sn_min_list=sn_min_list, sn_max_list=sn_max_list, $
        subtract_maps=subtract_maps, $
        lf_decor_map = lf_decor_map, $
        no_output_map=no_output_map, prism=prism, $
        parity=parity, force_file=force_file, xml = xml, $
        kill_subscan = kill_subscan, show_maps_only=show_maps_only, results_filing=results_filing, $
        lab=lab, input_polar_maps=input_polar_maps, lkg_kernel=lkg_kernel, nas_center=nas_center, $
        list_detector=list_detector, xguess=xguess, yguess=yguess, polar=polar, $
...
;;      info.error_report_file = param.project_dir+"/error_report_"+scan_list[iscan]+".dat"
      ...
      if defined(param) eq 1 then preproc_copy=param.preproc_copy else preproc_copy=0
      nk_update_param_info, scan_list[iscan], param, info, xml=xml, katana=katana, raw_acq_dir=raw_acq_dir, preproc_copy=preproc_copy
      ;; end LP modif
...
;      info.error_report_file = error_report_file
      ...
      if param.project_white_noise_nefd eq 0 then begin
         ...
         case param.clean_data_version of
            ...
            else: message, /info, "Wrong value of param.clean_data_version: "+ $
                           strtrim(param.clean_data_version,2)
         endcase
...
;; stop
      ...
      if param.keep_one_hwp_position ne 0 then $
         nk_keep_one_hwp_position, param, info, data

      nk_projection_4, param, info, data, kidpar, grid

      ;; If requested, make sure the background is zero on average
      if param.map_bg_zero_level_radius gt 0.d0 then begin
         kgauss = get_gaussian_kernel( !nika.fwhm_nom[0], grid.map_reso, /nonorm); PSF Gaussian kernel
         if param.k_snr_method eq 3 then begin
            ...
            nk_ata_fit_beam2, grid.map_i_1mm, grid.map_var_i_1mm, kgauss, info, $
                              flux_kgauss, sigma_flux_kgauss ; map of flux and error
         endelse
         snr_mask = grid.map_i1*0.d0+1D5
         w = where( sigma_flux_kgauss ne 0, nw)
         if nw ne 0 then snr_mask[w] = abs( flux_kgauss[w]/sigma_flux_kgauss[w])
                                ; FXD, correct from le to ge (avoid
                                ; the center to compute the background)
         w = where( sqrt( grid.xmap^2 + grid.ymap^2) ge param.map_bg_zero_level_radius and $
                    snr_mask lt 3. and grid.map_var_i1 gt 0. and grid.map_var_i3 gt 0. and $
                    grid.map_var_i_1mm gt 0., nw)
         if nw ne 0 and param.map_bg_zero_level_radius ge 0.d0 then begin
            grid.map_i1    -= total( grid.map_i1[w]/grid.map_var_i1[w], /double) / $
                              total( 1./grid.map_var_i1[w], /double)
            grid.map_i3    -= total( grid.map_i3[w]/grid.map_var_i3[w], /double) / $
                              total( 1./grid.map_var_i3[w], /double)
            grid.map_i_1mm -= total( grid.map_i_1mm[w]/grid.map_var_i_1mm[w], /double) / $
                              total( 1./grid.map_var_i_1mm[w], /double)
            ;; grid.map_i1    -= median( grid.map_i1[w])   ; median is safer than avg
            ;; grid.map_i3    -= median( grid.map_i3[w])
            ;; grid.map_i_1mm -= median( grid.map_i_1mm[w])
         endif

         kgauss = get_gaussian_kernel( !nika.fwhm_nom[1], grid.map_reso, /nonorm); PSF Gaussian kernel

                                ; Make sure the 2mm map is correct
         grid.map_i_2mm = grid.map_i2
         grid.map_var_i_2mm = grid.map_var_i2
         grid.nhits_2mm = grid.nhits_2
         if param.k_snr_method eq 3 then begin
            nk_ata_fit_beam3, grid.map_i_2mm, grid.map_var_i_2mm, kgauss, info, $
                              flux_kgauss, sigma_flux_kgauss ; map of flux and error
         endif else begin                                    ; all other cases
            nk_ata_fit_beam2, grid.map_i_2mm, grid.map_var_i_2mm, kgauss, info, $
                              flux_kgauss, sigma_flux_kgauss ; map of flux and error
         endelse

         snr_mask = grid.map_i_2mm*0.d0
         w = where( sigma_flux_kgauss ne 0, nw)
         if nw ne 0 then snr_mask[w] = abs( flux_kgauss[w]/sigma_flux_kgauss[w])
         w = where( sqrt( grid.xmap^2 + grid.ymap^2) ge $
                    param.map_bg_zero_level_radius $
                    and snr_mask lt 3 and grid.map_var_i_2mm gt 0., nw)
         if nw ne 0 then begin
            grid.map_i_2mm    -= total( grid.map_i_2mm[w]/grid.map_var_i_2mm[w], /double) / $
                                 total( 1./grid.map_var_i_2mm[w], /double)
;;;FXD median is not strong enough            grid.map_i_2mm    -= median( grid.map_i_2mm[w])
         endif
         grid.map_i2 = grid.map_i_2mm
      endif
