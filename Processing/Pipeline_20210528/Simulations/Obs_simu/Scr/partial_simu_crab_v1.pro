...
source = 'Simulation of the Crab Nebula'               ;Name of the source
version = 'v1'                                         ;Version of the analysis
name4file = STRJOIN(STRSPLIT(source,/EXTRACT),'_')     ;Name without space but '_'
coord_pointing = {ra:[5,34,31.95],dec:[22,0,52.1]} ;Pointing coordinates
coord_source = {ra:[5,34,31.95],dec:[22,0,52.1]}   ;Center of the source coordinates
...
partial_simu_default_param, param, 'given_map'       ;and add simulation params to the structure
...
param.caract_source.mapfile = !nika.save_dir+'/The_Crab_Nebula/astrometry_The_Crab_Nebula_v1.fits'

param.decor.method = 'COMMON_MODE_KIDS_FAR'
param.map.reso = 5
...
nika_pipe_launch, param, map_combi, map_list, /simu, /save

param.decor.method = 'COMMON_MODE_KIDS_OUT_MAP'
param.decor.common_mode.map_guess = output_dir+'/astrometry_'+name4file+'_'+version+'.fits'
param.decor.common_mode.flag_lim = [0.3,0.05]
nika_pipe_launch, param, map_combi, map_list, /simu, /save
...
zer_a = mean(map_a[where(rmap gt 250.0 and var_a gt 0)])
zer_b = mean(map_b[where(rmap gt 250.0 and var_b gt 0)])
...
fov_plot = 800.0                          ;Field of view of the nice plot
reso_plot = 4.0                           ;Resolution of the nice plot
...
overplot_radec_bar_map, map_plot_a, header, map_plot_a, header, fov_plot, reso_plot, coord_plot,$
                        ...
                        ;anothick1=3,$         
                        beam=sqrt(12.5^2+relob^2),/type, bg1=10*max(range_plot_a)
...
;;------- Jack-Knife map
map_jk = nika_pipe_jackknife(param, map_list)
