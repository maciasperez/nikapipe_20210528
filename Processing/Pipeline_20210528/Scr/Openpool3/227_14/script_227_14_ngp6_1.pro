...
pro script_222_14_ngp6_1, nscans=nscans, list_results_only=list_results_only, keep_current_result=keep_current_result, $
                          subtract_maps=subtract_maps, preproc=preproc, average=average, reset=reset


if keyword_set(average) then begin
rfile = !nika.plot_dir+'/running_227_14_ngp6_1.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('NGP6_1') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing NGP6_1 was found in '+db_file
   goto, exit
endif
...
param.name4file = 'NGP6_1'
param.project_dir = !nika.plot_dir+'/227_14/'+param.source
...
param.set_zero_level_per_subscan = 0 ;1
...
param.flag_uncorr_kid  = 1      ; ditto
param.w8_per_subscan   = 1
param.delete_all_windows_at_end = 0
param.discard_outlying_samples = 1
...
param.lf_sin_fit_n_harmonics  = 0
param.polynomial              = 0
param.fake_lissajous_subscans = 1
param.decor_elevation         = 1
param.nsample_min_per_subscan = 2

;param.project_dir = !nika.plot_dir+'/NGP6_1_common_mode_kids_out_and_filter'
...
param.fourier_opt_sample = 1
param.flag_uncorr_kid    = 1      ; ditto
param.iterate_uncorr_kid = 1 ; 2
param.discard_outlying_samples = 1

nk_default_info, info
nk_init_grid, param, grid
nk_default_mask, param, info, grid, dist=15.
...
if keyword_set(keep_current_result) then begin
   file = strtrim(param.project_dir,2)+'/MAPS_1mm_'+strtrim(param.name4file,2)+$
          '_v'+strtrim(param.version,2)+'.fits'
...
;;message, /info, "fix me:"
;;;scan_list = scan_list[0:14]
;;scan_list = scan_list[14:*]
;;stop

if keyword_set(preproc) then begin
;;   param.plot_png = 0
;;   param.plot_ps  = 0
;; for i=0, n_elements(scan_list)-1 do print, i, " ", scan_list[i]
;; stop
;;    nk, scan_list[15], subtract_maps=subtract_maps, $
;;        param=param, $
;;        filing=filing, $
;;        info = info, grid = grid, xml = xml, results_filing=results_filing, $
;;        parity=parity, simpar=simpar
;; print, "HERE"
;; stop
