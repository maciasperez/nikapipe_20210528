pro script_181_14_irc_plus_10216, nscans=nscans, list_results_only=list_results_only, keep_current_result=keep_current_result, $
subtract_maps=subtract_maps, preproc=preproc, average=average, reset=reset
if keyword_set(average) then begin
rfile = !nika.plot_dir+'/running_181_14_irc_plus_10216.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('IRC+10216') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing IRC+10216 was found in '+db_file
   goto, exit
endif
...
param.source        = 'IRC+10216'
param.silent            = 1
param.map_reso          = 2.d0 ; arcsec
param.name4file = 'IRC+10216'
param.project_dir = !nika.plot_dir+'/181_14/'+param.source
...
nk_default_mask, param, info, grid, dist=40
