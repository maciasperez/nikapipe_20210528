pro script_181_14_jupiter, nscans=nscans, list_results_only=list_results_only
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('JUPITER') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing JUPITER was found in '+db_file
   goto, exit
endif
...
param.source        = 'JUPITER'
param.silent            = 1
param.map_reso          = 2.d0 ; arcsec
param.name4file = 'JUPITER'
param.project_dir = !nika.plot_dir+'/181_14'
...
param.output_dir = param.project_dir
 nk_map2fits, param, info, output_maps, suffix='iter_0', $
              output_file_1mm=output_file_1mm, output_file_2mm=output_file_2mm
niter = 1
