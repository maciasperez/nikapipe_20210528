...
imrange_1mm = [-0.1,1]*30    ; Guess the range in Jy/Beam
imrange_2mm = [-0.1,1]*15
mapsize = 300. ; Map size in arcsec
avoid_list = [ '20150127s211']  ; 211 gives bad map
;avoid_list = ['20150127s211'] 213; too opaque
; 42.8, 16.8 Jy for [2]
...
;scan_list = scan_list[2:3]
; scan_list = scan_list[4:5]
;scan_list = scan_list[2]
; [0] to be redone with a reset
scanl = scanl[0:13]  ; DONE OK
nscans = n_elements( scanl)
for isc = 0, nscans-1 do begin
;for isc = 4, nscans-1 do begin
scan_list = scanl[isc]
print, '------------------------------'
...
param.plot_ps = 1  ; Make ps files NEED TO COMPILE PS PSOUT from MAMLIB
...
param.fine_pointing =  0 ; 1 not really necessary
...
nk_default_mask, param, info, grid, d = 1.5*!nika.fwhm_nom[1]
...
if nw gt 0 then junk[w] = out.map_var_i_2mm[w]
map_var_conv = convolve( junk, kernel^2)
;; Signal/Noise smoothed map
if nw gt 0 then map_sn_2mm_smooth[w] = map_conv[w]/sqrt( map_var_conv[w])

map_conv = convolve( out.map_i_1mm, kernel)
junk = out.map_i_1mm*0.d0
if nw gt 0 then w = where( finite( out.map_var_i_1mm) eq 1 and out.map_var_i_1mm ne 0, nw)
junk[w] = out.map_var_i_1mm[w]
map_var_conv = convolve( junk, kernel^2)
if nw gt 0 then map_sn_1mm_smooth[w] = map_conv[w]/sqrt( map_var_conv[w])
