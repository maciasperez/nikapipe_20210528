pro script_test, out_param, scan_list, nscans=nscans, $
                 list_results_only=list_results_only, keep_current_result=keep_current_result, $
                 map4iter=map4iter, preproc=preproc, average=average, reset=reset, iter=iter
;; if keyword_set(average) then begin
;; rfile = !nika.plot_dir+'/running_200_14_um311.dat'
...
nscans = 0 ; init
restore, db_file
w = where( strupcase( strtrim(scan.object,2))  eq strupcase('UM311') and $
...
if nw eq 0 then begin
   message, /info, 'No scan observing UM311 was found in '+db_file
   goto, exit
endif
...
param.name4file = 'UM311'
param.project_dir = !nika.plot_dir+'/Junk'
param.plot_dir    = param.project_dir+'/Plots'
param.preproc_dir = param.project_dir+'/Preproc'
param.up_dir      = param.project_dir+'/UP_files'
if keyword_set(iter) then param.up_dir = param.project_dir+'/UP_files_iter_'+strtrim(iter,2)
...
if keyword_set(map4iter) then begin
   subtract_map_file = !nika.plot_dir+'/output_maps_'+strtrim(param.name4file,2)+'_iter_'+strtrim(iter-1,2)+'.save'
   if file_test(subtract_map_file) then begin
      for i=0,2 do message, /info, "found "+subtract_map_file
...
if keyword_set(average) then begin
   ...
   outfile = !nika.plot_dir+'/output_maps_'+strtrim(param.name4file,2)+'.save'
   save, param, output_maps, grid, file=outfile
   if keyword_set(iter) then spawn, "cp "+outfile+" "+$
                                    !nika.plot_dir+'/output_maps_'+$
                                    strtrim(param.name4file,2)+'_iter'+strtrim(iter,2)+'.save'
