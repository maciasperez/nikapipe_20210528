pro flux_density_ratio_secondary_photocorr, png=png, ps=ps, $
   fwhm_stability=fwhm_stability, $
   obstau_stability=obstau_stability, $
   mwc349_stability=mwc349_stability, $
   photocorr_demo=photocorr_demo, $
   photocorr_pointing=photocorr_pointing
  ...
  ;; using recalibration factors produced in flux_density_ratio_primary_photocorr.pro
  local_calibration_factors = 1
  ...
  ;;________________________________________________________________
  ;;________________________________________________________________
  ;; if no available recalibration factors, set no_recalibration=1 
  no_recalibration=0
  if local_calibration_factors gt 0 then no_recalibration=1
  ...
  ;; Absolute calibration using local calibration factors
  if local_calibration_factors gt 0 then begin
     
     for irun=0, nrun-1 do begin
        w = where(runid eq calib_run[irun], nn)
        if keyword_set(photocorr_pointing) then $
           w = where(runid eq calib_run[irun] and index_baseline gt 0, nn)
        
        print, ''
        print, 'Calibration of ',calib_run[irun]
        calibration_file = !nika.soft_dir+'/Labtools/LP/datamanage/Calibration_coefficients_'+calib_run[irun]+'_LP_to_'+suf+'.save'
        print, 'RESTORING ', calibration_file
        restore, calibration_file
        
        ;; recalibrate
        flux_1mm[w] = flux_1mm[w]*calibration_coef[3]
        flux_a1[w]  = flux_a1[w]*calibration_coef[0]
        flux_a2[w]  = flux_a2[w]*calibration_coef[1]
        flux_a3[w]  = flux_a3[w]*calibration_coef[2]
  ...
  if keyword_set(mwc349_stability) then begin
     ...
     outfile = dir+'plot_flux_density_ratio_MWC349_obstau_secondary'+suf+'_1mm'
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[w_total] eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_1mm[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_1mm[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
        print, 'nscan = ', nn
        print, 'bias = ', mean(flux_ratio_1mm[w_total[w]])
        print, 'rel.rms = ', stddev(flux_ratio_1mm[w_total[w]])/mean(flux_ratio_1mm[w_total[w]])*100
     endfor
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_MWC349_obstau_secondary'+suf+'_a1'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[w_total] eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_a1[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a1[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
        print, 'nscan = ', nn
        print, 'bias = ', mean(flux_ratio_a1[w_total[w]])
        print, 'rel.rms = ', stddev(flux_ratio_a1[w_total[w]])/mean(flux_ratio_a1[w_total[w]])*100
     endfor
     ;;
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_MWC349_obstau_secondary'+suf+'_a3'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[w_total] eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_a3[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a3[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
        print, 'nscan = ', nn
        print, 'bias = ', mean(flux_ratio_a3[w_total[w]])
        print, 'rel.rms = ', stddev(flux_ratio_a3[w_total[w]])/mean(flux_ratio_a3[w_total[w]])*100
     endfor
     ;;
     legendastro, calib_run, col=col_tab, textcol=0, box=0, charsize=1., pos=[0.05, ymin+0.07], psym=cgsymcat('FILLEDCIRCLE', thick=2)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_MWC349_obstau_secondary'+suf+'_a2'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        ...
        print, 'nscan = ', nt
        w = where(runid[w_total] eq calib_run[irun], nn)
        if nn gt 0 then oplot, h_tau_a2[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a2[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
        print, 'nscan = ', nn
        print, 'bias = ', mean(flux_ratio_a2[w_total[w]])
        print, 'rel.rms = ', stddev(flux_ratio_a2[w_total[w]])/mean(flux_ratio_a2[w_total[w]])*100
     endfor
     ;;
     ...
     print, 'selected nscan = ', n_elements(w_total)
     
     print, 'A1 bias = ', mean(flux_ratio_a1[w_total])
     print, 'A3 bias = ', mean(flux_ratio_a3[w_total])
     print, '1mm bias = ', mean(flux_ratio_1mm[w_total])
     print, 'A2 bias = ', mean(flux_ratio_a2[w_total])
     
     print, 'A1 rel.rms = ', stddev(flux_ratio_a1[w_total])/mean(flux_ratio_a1[w_total])*100
     print, 'A3 rel.rms = ', stddev(flux_ratio_a3[w_total])/mean(flux_ratio_a3[w_total])*100
     print, '1mm rel.rms = ', stddev(flux_ratio_1mm[w_total])/mean(flux_ratio_1mm[w_total])*100
     print, 'A2 rel.rms = ', stddev(flux_ratio_a2[w_total])/mean(flux_ratio_a2[w_total])*100
     stop
  ...
  if keyword_set(obstau_stability) then begin
     ...
     n_total    = n_elements(index_baseline)
     w_total    = indgen(n_total) 
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_obstau_secondary'+suf+'_1mm'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, tau_1mm[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_1mm[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_obstau_secondary'+suf+'_a1'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, tau_a1[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a1[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_obstau_secondary'+suf+'_a3'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, tau_a3[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a3[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
     legendastro, sources, col=col_tab, textcol=0, box=0, charsize=1., pos=[0.05, ymin+0.10], psym=cgsymcat('FILLEDCIRCLE', thick=2)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_obstau_secondary'+suf+'_a2'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, h_tau_a2[w_total[w]]/sin(elev[w_total[w]]), flux_ratio_a2[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
  ...
  if keyword_set(fwhm_stability) then begin
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_FWHM_secondary'+suf+'_1mm'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, fwhm_1mm[w_total[w]], flux_ratio_1mm[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ...
     xyouts, xmin+(xmax-xmin)*0.1, ymax-(ymax-ymin)*0.1, 'A1&A3', col=0 
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_FWHM_secondary'+suf+'_a1'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, fwhm_a1[w_total[w]], flux_ratio_a1[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_FWHM_secondary'+suf+'_a3'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, fwhm_a3[w_total[w]], flux_ratio_a3[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
     legendastro, sources, col=col_tab, textcol=0, box=0, charsize=1., pos=[xmin+0.2, ymin+0.07], psym=cgsymcat('FILLEDCIRCLE', thick=2)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_3sources_FWHM_secondary'+suf+'_a2'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for isou=0, nsource-1 do begin
        w = where(obj[w_total] eq sources[isou], nn)
        if nn gt 0 then oplot, fwhm_a2[w_total[w]], flux_ratio_a2[w_total[w]], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[isou]
     endfor
     ;;
