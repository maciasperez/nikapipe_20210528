pro flux_density_ratio_primary_photocorr, png=png, ps=ps, $
  ...
  ;; will estimate new recalibration factors
  get_calibration_factors = 1
  ...
  no_recalibration=0
  if get_calibration_factors gt 0 then no_recalibration=1
  ...
  suf = '_photocorr'
  if keyword_set(photocorr_demo) then suf='photocorr_demo'
  if keyword_set(photocorr_pointing) then suf='photocorr_pointing'
  ...
  if get_calibration_factors gt 0 then begin

     photocorr_suf = ''
     if keyword_set(photocorr_demo) then photocorr_suf = '_photocorr_demo'
     if keyword_set(photocorr_pointing) then photocorr_suf = '_photocorr_pointing'
     ...
     for irun=0, nrun-1 do begin
        ...
        print, ''
        print, 'Calibration coef for ',calib_run[irun]
        calibration_coef = dblarr(4)+1.0d0
        if nn gt 0 then begin
           c_a1  = 1.0d0/mean(flux_ratio_a1[w])
           c_a2  = 1.0d0/mean(flux_ratio_a2[w])
           c_a3  = 1.0d0/mean(flux_ratio_a3[w])
           c_1mm = 1.0d0/mean(flux_ratio_1mm[w])
           calibration_coef = [c_a1, c_a2, c_a3, c_1mm]
        endif
        print, 'Calibration coef = ', calibration_coef
        calibration_file = !nika.soft_dir+'/Labtools/LP/datamanage/Calibration_coefficients_'+calib_run[irun]+'_LP_to_'+photocorr_suf+'.save'
        print, 'WRITING ', calibration_file
        save, calibration_coef, filename = calibration_file
  ...
  if keyword_set(fwhm_stability) then begin
     ...
     outfile = dir+'plot_flux_density_ratio_primary'+suf+'_1mm'
     ...
     for u = 0, nut-1 do begin
        ...
        if nn gt 0 then oplot, fwhm_1mm[w], flux_ratio_1mm[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=ut_col[u]
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_primary'+suf+'_a1'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for u = 0, nut-1 do begin
        w=where(ut_float ge ut_tab[u] and ut_float lt ut_tab[u+1], nn)
        if nn gt 0 then oplot, fwhm_a1[w], flux_ratio_a1[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=ut_col[u]
        ;;w=where(ut_float[w_baseline] ge ut_tab[u] and ut_float[w_baseline] lt ut_tab[u+1], nn)
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_primary'+suf+'_a3'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for u = 0, nut-1 do begin
        w=where(ut_float ge ut_tab[u] and ut_float lt ut_tab[u+1], nn)
        if nn gt 0 then oplot, fwhm_a3[w], flux_ratio_a3[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=ut_col[u]
        ;;w=where(ut_float[w_baseline] ge ut_tab[u] and ut_float[w_baseline] lt ut_tab[u+1], nn)
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_primary'+suf+'_a2'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for u = 0, nut-1 do begin
        w=where(ut_float ge ut_tab[u] and ut_float lt ut_tab[u+1], nn)
        if nn gt 0 then oplot, fwhm_a2[w], flux_ratio_a2[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=ut_col[u]
        ;;w=where(ut_float[w_baseline] ge ut_tab[u] and ut_float[w_baseline] lt ut_tab[u+1], nn)
  ...
  if keyword_set(obstau_stability) then begin
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_obstau_primary'+suf+'_1mm'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        w = where(runid eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_1mm[w]/sin(elev[w]), flux_ratio_1mm[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
     endfor
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_obstau_primary'+suf+'_a1'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        w = where(runid eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_a1[w]/sin(elev[w]), flux_ratio_a1[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
     endfor
     ;;
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_obstau_primary'+suf+'_a3'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        w = where(runid eq calib_run[irun], nn)
        if nn gt 0 then oplot, tau_a3[w]/sin(elev[w]), flux_ratio_a3[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
     endfor
     ;;
     legendastro, calib_run, col=col_tab, textcol=0, box=0, charsize=1., pos=[xmin+(xmax-xmin)*0.05, 1.17], psym=cgsymcat('FILLEDCIRCLE', thick=2)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
     ...
     wind, 1, 1, /free, xsize=600, ysize=400 
     outfile = dir+'plot_flux_density_ratio_obstau_primary'+suf+'_a2'
     outplot, file=outfile, png=png, ps=ps, xsize=12, ysize=8, charsize=1, thick=2, charthick=1.2
     ...
     for irun=0, nrun-1 do begin
        w = where(runid eq calib_run[irun], nn)
        if nn gt 0 then oplot, h_tau_a2[w]/sin(elev[w]), flux_ratio_a2[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_tab[irun]
     endfor
     ;;
     legendastro, calib_run, col=col_tab, textcol=0, box=0, charsize=1., pos=[xmin+(xmax-xmin)*0.05, 1.17], psym=cgsymcat('FILLEDCIRCLE', thick=2)*[1., 1., 1.]
     oplot, [xmin,xmax], [1., 1.], col=0
