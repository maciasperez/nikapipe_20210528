...
nba = 15
nscans = n_elements( scan)      ; should be 288
;; split_for, 0, nba-1, nsplit=nba, $
;;                  commands=['make_pointing_corr_n2r10_split1, i']
...
for isc = 0, nscans-1 do begin
   ...
   if size( /typ,  info1) eq 8 then begin
...
@make_pointing_corr_n2r10_add1.scr
nba = 15
nscans = n_elements( scan)      ; should be 314
split_for, 0, nba-1, nsplit=nba, $
                 commands=['make_pointing_corr_n2r10_split2, i']
; done with PF, and no gain elevation curve
;------------------------------------------------------
...
; Retrieve the v1 pointing corrections
nk_default_param, param
v1 = '2' ; '2' without correction, '3' after pointing correction
v2 = '3'
for isc = 0, nscans-1 do begin
   param.project_dir = param.dir_save+ '/All'
...
filecsv_out = '$NIKA_SOFT_DIR/Pipeline/Datamanage/Logbook/' + $
        'Log_Iram_corr_N2R10_v1.csv'   ; This file is used in nk_get_kidpar_ref
