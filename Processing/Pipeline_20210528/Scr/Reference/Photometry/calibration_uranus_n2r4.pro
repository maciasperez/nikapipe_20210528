;
;  repeat the analysis of N2R4 with the same tools as N2R12
;
;  created on Nov. 15, 2017
;____________________________________________________________________


pro calibration_uranus_n2r4, input_kidpar_file, nickname, $
                             compute=compute, png=png, ps=ps, $
                             outplot_dir=outplot_dir
  
  nk_get_kidpar_ref, scan_num, day, info, kidpar_file, $
                     scan='20160122s1'
  ;; reference kidpar is !nika.off_proc_dir+"/kidpar_20160127s202_203_204_3C84_MWC349.fits"
  
  input_kidpar_file = !nika.off_proc_dir+"/kidpar_20160127s202_203_204_3C84_MWC349.fits"
  nickname          = 'N2R4v1'
  ...
  png               = 1
  ps                = 0
  outplot_dir       = '/home/perotto/NIKA/Plots/N2R4/Photometry'
  no_tel_gain_correction = 1 ;; no telescope elevation-gain correction
  ...
  restore, !nika.pipeline_dir+"/Datamanage/Logbook/Log_Iram_tel_Run16_v0.save"
  ...
  wtokeep = where(strupcase(scan[wok].obstype) eq 'ONTHEFLYMAP' $
                  ...
                  and scan[wok].el_deg gt 45.0d0, nkeep)
     ...
     for NoTauCorrect=0, check_taucorrect do begin
        ...
        split_for, 0, nscans-1, $
                   commands=['obs_nk_ps, i, scan_list, project_dir, '+$
                             ...
                             'no_tel_gain_correction=no_tel_gain_correction'], $
                   nsplit=nproc, $
                   varnames=['scan_list', 'project_dir', 'method', 'source', 'input_kidpar_file', $
                             'reset', 'NoTauCorrect', 'no_tel_gain_correction']
     endfor
  endif
...
;; check if all scans were indeed processed
  ...
  for NoTauCorrect=0, check_taucorrect do begin
     ...
     wtokeep = where( abs(fwhm[*, 0]-fwhm_avg[0]) le 2.5*fwhm_sigma[0] and $
                      abs(fwhm[*, 1]-fwhm_avg[1]) le 2.5*fwhm_sigma[1] and $
                      abs(fwhm[*, 2]-fwhm_avg[2]) le 2.5*fwhm_sigma[2], compl=wout, nscans, ncompl=nout)
     ...
     for j=0, 2 do begin
        ...
        if j eq 2 then   xyouts, index[wtokeep], fwhm[wtokeep,j], scan_list[wtokeep], charsi=0.5, orient=90
        if nout gt 0 and j eq 2 then  xyouts, index[wout], fwhm[wout,j], scan_list[wout], charsi=0.5, orient=90, col=250
        myday = day_list[0]
        for i=0, nscans-1 do begin
     ...
     yra=!nika.flux_uranus[0]*[0.6, 1.4]
     ...
     yra=!nika.flux_uranus[1]*[0.6, 1.4]
     ...
     xyouts, index, flux_2, strmid(scan_list,4, 12), charsi=0.5, orient=90
