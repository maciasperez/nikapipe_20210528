...
pro crosscheck_using_calibrators_n2r14


input_kidpar_file = !nika.off_proc_dir+"/kidpar_20180117s92_v2_LP_skd13_calUranus12.fits"
nickname          = 'refkidpar'

input_kidpar_file = !nika.off_proc_dir+"/kidpar_20180117s92_v2_LP_skd16_calUranus16.fits"
nickname          = 'calibv2'

input_kidpar_file = !nika.off_proc_dir+"/kidpar_20180117s92_v2_LP_skd14_calUranus8.fits"
nickname          = 'calibv3'

input_kidpar_file = !nika.off_proc_dir+"/kidpar_20180122s309_v2_HA_skd16_calUranus17.fits"
nickname          = 'calibv4'

input_kidpar_file = '/home/perotto/NIKA/Plots/N2R14/Photometry/kidpar_20180122s309_v2_HA_avril_skd13_calUranus14.fits'
nickname          = 'calibv5'

;;input_kidpar_file = '/home/perotto/NIKA/Plots/N2R14/Photometry/kidpar_20180122s309_v2_HA_avril_skd13_hybrid_calUranus14.fits'
;;nickname          = 'calibv6'

;;input_kidpar_file = '/home/perotto/NIKA/Plots/N2R14/Photometry/kidpar_20180122s309_v2_HA_avril_skd13_hybrid_calUranus41.fits'
;;nickname          = 'calibv7'
...
;; FWHM selection cut (if photocorr=0)
fwhm_a1_max = 12.0
fwhm_a3_max = 12.0
fwhm_a2_max = 17.9
...
do_tel_gain_corr  = 2 ;; NIKA2 telescope elevation-gain correction (if photocorr=0)
...
for NoTauCorrect=0, check_taucorrect do begin
   ...
   if photocorr gt 0 then begin
      ...
      outplot, file='Photometric_correction_'+strtrim(source)+'_'+strtrim(nickname,2), $
                 png=png, ps=ps
      ...
      new_calib = [0.9461,     1.031,     0.9693]
      old_calib = [0.95430035, 1.0333255, 0.96902114 ]
      raw_flux = flux
      for ia = 0, 2 do flux[*, ia] = flux[*, ia]*corr_flux_factor[ia,*]*new_calib[ia]/old_calib[ia]
   ...
   plot, tau_1mm[*]/sin(elev[*]*!dtor) , flux[*, 0], /xs, yr=[0.7, 2.3], $
         xr=[0.1, 0.7], psym=-4, $
         xtitle='obs. opacity', ytitle='Flux density (Jy/beam)', /ys, /nodata
   
   oplot, tau_1mm[wtokeep]/sin(elev[wtokeep]*!dtor) , flux[wtokeep, 2], psym=8, col=coltab[2]
   oplot, tau_1mm[wout]/sin(elev[wout]*!dtor) , flux[wout, 2], psym=4, col=coltab[2]
   if photocorr gt 0 then oplot, tau_1mm[wtokeep]/sin(elev[wtokeep]*!dtor) , raw_flux[wtokeep, 2], psym=8, col=150
    if photocorr gt 0 then oplot, tau_1mm[wout]/sin(elev[wout]*!dtor) , raw_flux[wout, 2], psym=4, col=150
   ...
   wind, 1, 1, /free, xsize=800, ysize=400
   plot, tau_2mm[*]/sin(elev[*]*!dtor) , flux[*, 1], /xs, yr=[0.3, 1.5], $
         xr=[0.1, 0.6], psym=-4, $
         xtitle='obs. opacity', ytitle='Flux density (Jy/beam)', /ys, /nodata
   
   oplot, tau_2mm[wtokeep]/sin(elev[wtokeep]*!dtor) , flux[wtokeep, 1], psym=8, col=coltab[1]
   oplot, tau_2mm[wout]/sin(elev[wout]*!dtor) , flux[wout, 1], psym=4, col=coltab[1]
    if photocorr gt 0 then oplot, tau_2mm[wtokeep]/sin(elev[wtokeep]*!dtor) , raw_flux[wtokeep, 1], psym=8, col=150
    if photocorr gt 0 then oplot, tau_2mm[wout]/sin(elev[wout]*!dtor) , raw_flux[wout, 1], psym=4, col=150
