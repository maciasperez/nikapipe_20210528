...
pro check_nefd_vs_observed_opacity, runname, outdir, png=png, ps=ps, pdf=pdf, $
  ...
  ;;________________________________________________________________
  ;;________________________________________________________________
  get_all_scan_result_file, allresult_file, outputdir = outdir
  ...
  save, recalibration_coef, file = 'recalibration_coef_new_opacity_'+runname+'.save'
                                ;stop
...
;;;-------------------------------------------------------------------
  ...
  nefd0_tab   = dblarr(4)
  nefdA_tab   = dblarr(4)
  rms_nefd0_tab   = dblarr(4)
  rms_nefdA_tab   = dblarr(4)
  ...
  oplot, obs_tau, nefd_1mm[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_n2r9,symsize=0.5
  
  ;; test
  wtest = where(nefd_1mm gt 0.0 and $
  ...
  nefd0_tab[3] = nefd_0
  nefdA_tab[3] = nefd_0*exp(atm_tau1/sin(60.0d0*!dtor))
  rms_nefd0_tab[3] = rms_nefd_0
  rms_nefdA_tab[3] = rms_nefd_0*exp(atm_tau1/sin(60.0d0*!dtor))
  print, 'NEFD_0 = ', nefd0_tab[3]
  print, 'NEFD 0 MOY = ', mean(nefd_1mm[w_atmtrans]*atm_trans[w_atmtrans])
  print, 'RMS NEFD_0 = ', rms_nefd_0 
  print, 'NEFD IRAM = ', nefdA_tab[3]
  print, 'RMS NEFD IRAM = ', rms_nefd_0*exp(atm_tau1/sin(60.0d0*!dtor)) 

  w_atmtrans = where(atm_trans gt 0.5 and atm_trans le 1.0 and nefd_1mm gt 0.0 and $
                     (ut_float le 9. or ut_float ge 22.), nn)
  print, 'NEFD_0 night = ', median(nefd_1mm[w_atmtrans]*atm_trans[w_atmtrans]) 
  
  oplot,obstau, nefd_0*exp(obstau), col=col_n2r9
  
  whigh = where(nefd_1mm*atm_trans gt nefd_0+3.*rms_nefd_0, nhigh)
  print, 'n high NEFD = ', nhigh
  if nhigh gt 0 then begin
     print, obj[whigh]
     print, ut[whigh]
     print, flux_1mm[whigh]
     print, "tau = ", tau_1mm[whigh]
     print, "el = ",  elev[whigh]/!dtor
  ...
  if nn gt 0 then oplot, obs_tau[w], nefd_a1[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_n2r9,symsize=0.5
  w_obstau = where(atm_trans gt 0.5 and atm_trans le 1.0 $
                   and nefd_a1 gt 0.0, nn)

  nefd_0 = median(nefd_a1[w_obstau]*atm_trans[w_obstau])
  rms_nefd_0 = stddev(nefd_a1[w_atmtrans]*atm_trans[w_atmtrans])
  nefd0_tab[0] = nefd_0
  nefdA_tab[0] = nefd_0*exp(atm_tau_a1/sin(60.0d0*!dtor))
  rms_nefd0_tab[0] = rms_nefd_0
  rms_nefdA_tab[0] = rms_nefd_0*exp(atm_tau_a1/sin(60.0d0*!dtor))
  print, 'nscans = ', nn
  print, 'NEFD_0 = ', nefd0_tab[0]
  print, 'NEFD_0 MOY = ', mean(nefd_a1[w_obstau]*atm_trans[w_obstau])
  print, 'RMS NEFD_0 = ', rms_nefd_0 
  print, 'NEFD IRAM = ', nefdA_tab[0]
  ...
  if nn gt 0 then oplot, obs_tau[w], nefd_a3[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_n2r9,symsize=0.5
  w_obstau = where(atm_trans gt 0.5 and atm_trans le 1.0 and $
                   nefd_a3 gt 0.0, nn)
  
  nefd_0 = median(nefd_a3[w_obstau]*atm_trans[w_obstau])
  rms_nefd_0 = stddev(nefd_a3[w_atmtrans]*atm_trans[w_atmtrans])
  nefd0_tab[2] = nefd_0
  nefdA_tab[2] = nefd_0*exp(atm_tau_a3/sin(60.0d0*!dtor))
  rms_nefd0_tab[2] = rms_nefd_0
  rms_nefdA_tab[2] = rms_nefd_0*exp(atm_tau_a3/sin(60.0d0*!dtor))
  print, 'nscans = ', nn
  print, 'NEFD_0 = ', nefd0_tab[2]
  print, 'NEFD_0 MOY = ', mean(nefd_a3[w_obstau]*atm_trans[w_obstau])
  print, 'RMS NEFD_0 = ', rms_nefd_0 
  print, 'NEFD IRAM = ', nefdA_tab[2]
  ...
  if nn gt 0 then oplot,obs_tau[w], nefd_a2[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_n2r9, symsize=0.5
  w_obstau = where(atm_trans gt 0.5 and atm_trans le 1.0 and $
                   nefd_a2 gt 0.0, nn)

  nefd_0 = median(nefd_a2[w_obstau]*atm_trans[w_obstau])
  rms_nefd_0 = stddev(nefd_a2[w_atmtrans]*atm_trans[w_atmtrans])
  nefd0_tab[1] = nefd_0
  nefdA_tab[1] = nefd_0*exp(atm_tau2/sin(60.0d0*!dtor))
  rms_nefd0_tab[1] = rms_nefd_0
  rms_nefdA_tab[1] = rms_nefd_0*exp(atm_tau2/sin(60.0d0*!dtor))
  print, 'nscans = ', nn
  print, 'NEFD_0 = ', nefd0_tab[1]
  print, 'NEFD_0 MOY = ', mean(nefd_a2[w_obstau]*atm_trans[w_obstau])
  print, 'RMS NEFD_0 = ', rms_nefd_0 
  print, 'NEFD IRAM = ', nefdA_tab[1]
  ...
  ;; SAUVEGARDE FICHIER
     ...
     if savefile gt 0 then begin
        ...
        for ia = 0, 3 do begin
           printf, lun, quoi[ia], ' nefd0 = ', nefd0_tab[ia], ', nefdA = ', nefdA_tab[ia], $
                   ' rms_nefd0 = ', rms_nefd0_tab[ia], ', rms_nefdA = ', rms_nefdA_tab[ia]
        endfor
     ...
     if nn gt 0 then oplot, flux_1mm[w], nefd_1mm[w], psym=cgsymcat('FILLEDCIRCLE', thick=2), col=col_n2r9,symsize=0.5
     w = where(nefd_1mm gt 0.0 and $
