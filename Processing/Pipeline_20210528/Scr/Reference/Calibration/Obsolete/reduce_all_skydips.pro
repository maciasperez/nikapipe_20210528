...
pro reduce_all_skydips, run, db_file, $
...
if not keyword_set(output_dir) then output_dir = !nika.plot_dir+"/"+run+"/Opacity"
spawn, "mkdir -p "+output_dir

;; set to 1 to derive a single opacity per band rather than an opacity
;; per matrix
opacity_per_band = 0
...
;; Determine the list of scans
if not keyword_set(skydip_scan_list) then begin
   restore, db_file
   db_scan = scan
   w = where( strupcase( db_scan.obstype) eq "DIY", nw)
   if nw eq 0 then begin
      message, /info, "No skydip done for run "+run+" ?!"
      stop
   endif
   skydip_scan_list = strtrim(db_scan[w].day,2)+"s"+strtrim(db_scan[w].scannum,2)
endif

;; Take the reference kidpar by default
if not keyword_set(kidpar_file) then begin
   nk_get_kidpar_ref, s, d, i, kidpar_file, scan=skydip_scan_list[0]
endif

;; message, /info, "fix me:"
;; skydip_scan_list = skydip_scan_list[0:3]
;; stop
...
if (redo_plot lt 1 and do_test lt 1) then begin
   skdout=1
   all_skydips, output_dir=output_dir, input_scan_list=skydip_scan_list, $
                reset=reset, goodscan = goodscan, kidparfile=kidpar_file, $
                run=run, file_suffixe=file_suffixe, $
                istart = istart, iend = iend, skdout=skdout, $
                use_atm=0, opacity_per_band=opacity_per_band, kidpar_out_file=kidpar_out_file
endif else begin 
   
   kidpar = mrdfits( kidpar_file, 1) ; mrdfits(!nika.off_proc_dir+'/'+kidpar_file,1)
   
   testkidpar_file = output_dir+'/kidpar_'+strupcase(run)+strtrim(file_suffixe, 2)+'_skydip.fits'
   print, "test kidpar = ", testkidpar_file
   newkidpar = mrdfits(testkidpar_file, 1)
   
   testsave_file = output_dir+'/all_skydip_fit_'+strupcase(run)+strtrim(file_suffixe,2)+'.save'
   ...
   if do_test gt 0 then begin
      ...
      outplot, file='Test_C0C1_tau1_tau2_ratio'+run, png=png

      xmin = min(skdout.taufinal1)-0.05

      if opacity_per_band gt 0 then begin
         
         plot, tau_recalc[*, 2], tau_recalc[*, 3]/tau_recalc[*, 2], $
               ytitle="zenith opacity 2mm-to-1mm ratio", xtitle="zenith opacity at 1mm", $
               /ys, /xs, /nodata, xr=[xmin, 0.8], yr=[0.2,1.]
         ...
         legendastro, ["pipe tau2/tau1", 'skydip tau2/tau1'], col=[200, 80], psym = [4, 8], $
                      symsize=[1.1,0.8], box=0, /trad, textcol=col, pos=[0.55, 0.95]
      endif else begin
         
         plot, tau_recalc[*, 0], tau_recalc[*, 3]/tau_recalc[*, 2], ytitle="zenith opacity 2mm-to-1mm ratio", $
               xtitle="zenith opacity at 1mm", /ys, /xs, /nodata, xr=[xmin, 0.8], yr=[0.2,1.]
         ...
         legendastro, ["pipe A2/A1", "pipe A2/A3", 'skydip A2/A1', 'skydip A2/A3'], $
                      col=[200, 250, 80, 50], psym = [4, 4, 8, 8], symsize=[1.1, 1.1, 0.8, 0.8], $
                      box=0, /trad, textcol=col, pos=[0.55, 0.95]
...
kpref = mrdfits(kidpar_file,1) ; mrdfits(!nika.off_proc_dir+'/'+kidpar_file,1)
kpnew = mrdfits(output_dir+'/kidpar_'+strupcase(run)+strtrim(file_suffixe)+'_skydip.fits', 1)
...
dir = output_dir
outplot, file=dir+'/Coefficient_checks_'+run+file_suffixe, png=png, ps=ps
