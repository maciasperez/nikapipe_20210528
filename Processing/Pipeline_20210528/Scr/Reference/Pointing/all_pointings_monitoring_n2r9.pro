...
pro all_pointings_monitoring_n2r9

;run = 'N2R9'
;restore, !nika.pipeline_dir+"/Datamanage/Logbook/Log_Iram_tel_N2R9_v0.save"

run = 'N2R10'
restore, !nika.plot_dir+'/Log_Iram_tel_' + run + '.save'

message, /info, "fix me:"
message, /info, "as it is, it assmes that all pointings were correct on profiles and applied"
message, /info, "this was not always the case, so I should rather look for changes in p2cor and p7cor"
stop
...
;   if file_test(log_file) eq 0 then nk_rta, scan_list[iscan], /noscp
   if file_test(log_file) eq 0 then begin
      if ii eq 0 then begin
         scan2reduce = [scan_list[iscan]]
         ii++
      endif else begin
         scan2reduce = [scan2reduce, scan_list[iscan]]
      endelse
   endif
endfor
...
if ii ne 0 then begin
   if nscans eq 1 then begin
      nk_rta, scan2reduce[0]
   endif else begin
      nscans = n_elements(scan2reduce)
      ncpu_max = 16
      optimize_nproc, nscans, ncpu_max, nproc
      split_for, 0, nscans-1, nsplit=nproc, $
                 commands = 'myrta, i, scan2reduce', varnames='scan2reduce'
   endelse
endif
...
;; scan selection
w = where( scan.obstype eq 'pointing' and scan.tiptau225ghz le 0.5, nw)
;;plot, scan[w].tiptau225ghz
scan_list = strtrim(scan[w].day,2)+"s"+strtrim(scan[w].scannum,2)
nscans = n_elements(scan_list)
