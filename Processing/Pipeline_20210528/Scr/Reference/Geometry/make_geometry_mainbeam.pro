...
pro make_geometry_mainbeam, scan_list, input_flux_th, nproc=nproc, black_list=black_list, $
                            ...
                            parallel=parallel, plateau=plateau, gamma=gamma, asymfast=asymfast, $
                            ...
                            sidelobe_mask_fit = side_lobe_mask_fit, guess_fit_par=guess_fit_par, interpol=interpol, $
                            use_fwhm_azel=use_fwhm_azel, dir_nickname=dir_nickname
...
if keyword_set(dir_nickname) then dirsuf = dir_nickname else dirsuf = 'kids_out'
...
if iteration eq 1 then begin
   ...
   beams_output_dir   = beam_maps_dir+"/Beams_"+dirsuf
   kidpars_output_dir = beam_maps_dir+"/Kidpars_"+dirsuf
   toi_dir            = beam_maps_dir+"/TOIs"
endelse
...
if keyword_set(beams) then begin
   geom_fit_mainbeam_parameters, nproc, maps_dir, beams_output_dir, kidpars_output_dir, nickname, input_flux_th, $
...
if keyword_set(select) then begin
   ...
   if keyword_set(no_ref_center) then begin
      kidpar_select_file = "kidpar_"+nickname+"_v"+strtrim(version, 2)+"_NoRefCenter.fits"
   endif else begin
      kidpar_select_file = "kidpar_"+nickname+"_v"+strtrim(version, 2)+".fits"
   endelse
