...
pro skydip_new, day, scan_num, kidpar, param=param, sav=sav, help=help, test=test, png=png, ps=ps, RF=RF, $
                no_acq_flag=no_acq_flag, force=force, conserv=conserv, imbfits=imbfits, antimb=antimb
...
nika_pipe_getdata, param, data, kidpar, /nocut, ext_params=ext_params, no_acq_flag=no_acq_flag
...
;
         ...
         ind_t1=where(tun gt 0,ntun,comp=notun,ncomp=nnotun)
        if ntun gt 1 then begin
          bs = lonarr(ntun+1)
          es = bs
          bs[0]=0
          es[ntun] = n_elements(tun)-1
          for itun = 0, ntun-1 do es[itun] = ind_t1[itun]-1
          for itun = 1, ntun do bs[itun] = ind_t1[itun-1]+der < es[ntun]
          toto = max(es-bs,loc)
          bidx = bs[loc]+backot[i]
          eidx = es[loc]+backot[i]
         endif else begin
; 
            if ntun eq 0 then ind_t1 = 0
            if ntun eq 1 then ind_t1 = ind_t1 + der
            bidx = backot[i] + ind_t1
            eidx = subscan_f[i]-1
         endelse

          ell[i]=mean(el[bidx:eidx])
          dft[j,i]=mean(df_tone[flagkid[j],bidx:eidx])
          ft(j,i)=mean(f_tone[flagkid[j],bidx:eidx])
          er(j,i)=sqrt(stddev(f_tone[flagkid[j],bidx:eidx])^2+stddev(df_tone[flagkid[j],bidx:eidx])^2)
...
;            window,4
;            !P.multi=[0,2,1]
;;            plot,reform(df_tone(flagkid(j),backot(i)+ ind_t1 : subscan_f(i)))
...
;   idx1=where(sqrt((tau-mean(tau))^2) lt 0.7*stddev(tau))
   idx1=where(sqrt((tau-mean(tau))^2) lt 3*stddev(tau))
   tau_v=tau(idx1)
   Const=K(idx1)
   f_to=fto(idx1)
;                                                                     
...
;; Save output
nika_write_kidpar, kidpar, !nika.save_dir+"/skydip_kidpar_"+strtrim(day,2)+"s"+strtrim(scan_num,2)+".fits"
;;stop
fin:
!p.multi=0
end
