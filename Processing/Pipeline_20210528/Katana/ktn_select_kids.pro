...
PRO ktn_double_kids_event, ev
  common ktn_common
  ktn_matrix_event, ev, action='double'
end


PRO ktn_select_kids, kidx=kidx, kidy=kidy, action = action
  common ktn_common
  ...
  xsize = long( disp.xsize_matrix + xs + 300) ; long( disp.xsize_matrix + xs + 20)
  ...
  ;; Create widget
  if not keyword_set(action) then action = 'discard'
  main = widget_base( title='Click to '+strtrim(action, 2)+' kids', /col, /frame)
     ...
     for j=0, n_elements(disp.plot_position[0,*,0])-1 do begin
        for i=0, n_elements(disp.plot_position[*,0,0])-1 do begin
           if ikid lt kmax then begin
              ...
              ;; LP imrange
              ;; NB: max=20000, well-suited for Uranus, TBC for another source
              ;;imrange = [-0.08*max(matrix[kmin:kmax-1,*,*]),min([0.2*max(matrix[kmin:kmax-1,*,*]),4000.])]
  ...
  if strupcase( strtrim(action, 2)) eq "DISCARD" then xmanager, "ktn_discard_kids", main, /no_block
  if strupcase( strtrim(action, 2)) eq "RESTORE" then xmanager, "ktn_restore_kids", main, /no_block
  if strupcase( strtrim(action, 2)) eq "DOUBLE"  then xmanager, "ktn_double_kids", main, /no_block
END
