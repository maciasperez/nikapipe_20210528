...
pro nika_map_noise_estim, param, map, xmap, ymap, fwhm, flux, $
...
nx_beam    = 2*long(3*sigma_beam/param.map.reso/2)+1
ny_beam    = 2*long(3*sigma_beam/param.map.reso/2)+1
xx         = dblarr(nx_beam, ny_beam)
yy         = dblarr(nx_beam, ny_beam)
for ii=0, nx_beam-1 do xx[ii,*] = (ii-nx_beam/2)*param.map.reso
for ii=0, ny_beam-1 do yy[*,ii] = (ii-ny_beam/2)*param.map.reso
...
bg_rms = sqrt( sigma_h^2/avg( nhits[wpix]))*param.map.reso/10. ; for 10 arcsec square, fiducial
