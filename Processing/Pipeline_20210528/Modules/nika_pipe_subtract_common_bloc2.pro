;+
;PURPOSE: usefule subroutine for nika_pipe_cmkidout. Same as
;nika_pipe_subtract_common_bloc.pro but mask only for regressing the template
;
...
pro nika_pipe_subtract_common_bloc2, param, rf_didq, kidpar, w8source, temp_atmo, base, $
...
;  w1  = where( kidpar.type eq 1 and total(w8source,2) gt 0.0, nw1)
  ...
  for i=0, nw1-1 do begin
     ...
     for j=0, nbloc-1 do begin
        if keyword_set(toi_Est) then cm_b += (atm_x_calib[bloc[j],0] + atm_x_calib[bloc[j],1]*(rf_didq[bloc[j],*] - toi_est[bloc[j], *])) else cm_b += (atm_x_calib[bloc[j],0] + atm_x_calib[bloc[j],1]*rf_didq[bloc[j],*])
        if keyword_set(toi_Est) then hit_b += w8source[bloc[j],*]*0+1 else hit_b += 1+0*w8source[bloc[j],*]
     endfor
