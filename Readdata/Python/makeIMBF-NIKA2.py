...
if __name__ == "__main__":
   ...
   for substr in ['continuum','NIKA','NIKA1mm','NIKA2mm']:
      if string.count(sys.argv[0],substr) > 0:
         ...
         print 'Do the '+ChosenBackend+' case'
         import imbfits_NIKA
   if ChosenBackend == 'none':
      print 'No supported Backend found '+ChosenBackend
      sys.exit(1)
...
### Should be set from input line
   ...
   print "Processing Array No.",NIKAarray
...
##   DataRootDir = '/media/sievers/nika2/'
   ...
   if obsBlock.allParams.has_key(key):
      ...
      print "...observingMode.receiverName:",obsBlock.allParams[key].dump() 
      receiverNameSSB = getNormValue(receiverNameParams.attrs)
      print "receiverName for SSB-cal :", receiverNameSSB
   #print "dir of obsBlock:", dir(obsBlock)
   #print obsBlock.allParams.keys()
   ...
   if found:
      ...
      print iYear,iMonth,iDay,sHours,sMinutes,sSeconds,siteTime,SDate
   else:
      print "Sync message expected from masterCS ",findKey
   ...
   print "The backend chosen is:",ChosenBackend
   ...
   if ChosenBackend == 'NIKA':
      OutName="iram30m-NIKA-%s-"%(NIKAarray) +InputFileStub+"-imb.fits"
      OutFITSfile = imbfits_NIKA.FitsFile("!"+outdir+OutName)
   ...
   ## edit Version
   pcfitsio.fits_update_key(fptr,'IMBFTSVE',10.1,'IMBFITS version')
   pcfitsio.fits_update_key(fptr,'CREATOR','Python IRAM MBFITS Writer v3.0 (c) 2013 IRAM','Software ')
   ## edit Date
   pcfitsio.fits_write_comment(fptr,'This Version: 13-04-2016')
   ...
   for Receiver in obsBlock.receivers.receiverList:
      ...
      print "used receiver ",receiverName
      if receiverName != 'Bolometer':
         print 'Can not process: ',receiverName
         exit(1)
   ## Fix me
...
#      Scan1.Header.updateKeyword(fptr,'WOBUSED','',wobblerUsed)
      ...
      print 'Unknown switching mode ',res3
   ...
   if not found:
      ...
      print "dUTC, dX, dY= ",ut1utc,polax,polay
#   (ut1utc,polax,polay)=EOP.get()
...
#   taiutc=35.0    # From I E R S  BULLETIN - A 11 April 2013 Vol. XXVI No. 015 
   ...
   try:
      ...
      print 'No source offsets'
   else:
      print 'Yes we have Offsets ',NoOffsets
      for iOff in range(NoOffsets):
         ...
         print Xoffset,Yoffset,SysOff,' from PaKo'
...
#      print Values1
      ...
      SysOff="Nasmyth"
      print Xoffset,Yoffset,SysOff,' from Walter'
      Scan1.BinTable.addTableEntry(fptr,'XOFFSET',[Xoffset])
      Scan1.BinTable.subsTableEntry(fptr,'YOFFSET',[Yoffset])
      Scan1.BinTable.subsTableEntry_str(fptr,'SYSOFF' ,[SysOff])
   ...
   if not found:
      ...
      print "antMD slow loops per sec ",slowRate
   ...
   if not found:
      ...
      print "Weather-station mode ",WSmode
   ...
   if not found:
      ...
      print 'heraTcold found ',heraTcold
   ...
   if not found:
      ...
      print 'Doppler correction found ',dopplerCorrection
   ...
   if not found:
      ...
      if valueDopplerVelocity != "":
         ...
         print "doppler Vsum, Vsource, Vlsr :",dopplerVsum,sourceVelocity,dopplerVlsr
      else:
         print "Empty string for doppler velocity"
         dopplerVlsr=0.0
      print 'Vlsr observer ',dopplerVlsr
...
# table
   if NIKAarray==1:
      ...
      print "No NIKA array no. found: ",NIKAarray
   Scan1.Header.updateKeyword(fptr,'RECON','',pointingChannel)
   Scan1.Header.updateKeyword(fptr,'RECCNTRL','',recCntrl)
   #   
...
#      import sla
#      print "SLALib wrapper from ",sla.__file__
      silent=1
...
#      list_data='subscan scan El retard 0 ofs_Az ofs_El Az paral scan_st MJD LST sample I Q dI dQ RF_didq F_tone DF_tone A_masq B_masq k_flag c_position c_synchro A_t_utc B_t_utc antxoffset antyoffset anttrackaz anttrackel'
      list_data='subscan scan El retard 0 ofs_Az ofs_El Az paral scan_st MJD LST sample I Q dI dQ RF_didq F_tone DF_tone A_masq B_masq k_flag c_position c_synchro A_t_utc B_t_utc C_t_utc D_t_utc E_t_utc F_t_utc G_t_utc H_t_utc I_t_utc J_t_utc K_t_utc L_t_utc M_t_utc N_t_utc O_t_utc P_t_utc Q_t_utc R_t_utc S_t_utc T_t_utc U_t_utc'
#      rawdata= read_nika_data(rawFile,silent,list_data,nika2run2)
      rawdata= read_nika_data(rawFile,silent,det2read='KID',list_data=list_data,nika2run2=nika2run2)
...
#      dectarr = nup.array(rawdata.kidpar['pvalue'][posA[0][0]])
      ...
      print nup.size(dect1array)
      ##print "Liste :",dect1array[0].tolist()
#      channelL=dect1array[0].tolist()
      channelL=dect1array.tolist()
      NIKAChannels=nup.size(channelL)
      print "NIKAChannels ",NIKAChannels
...
#               print NAME_VAR, DATA_VAR
               Febepar2.BinTable.subsTableEntry(fptr,NAME_VAR,[DATA_VAR])
               
   print 'No. of data per pixel expected for ',ChosenBackend,' are ',numBEparts
...
#      FeedAngl = obsBlock.resources['antenna'].resources['secondary'].values['rotationAngle']
   #   FeedAngl = scanSpecParser.getNormValue(observingBlock.allParams[key].attrs)
      ...
      print "Feed Angle not set, assume it is zero"
      FeedAngl=0.0
   else:
   #   FeedAngl = FeedAngl*180./math.pi is already in radian
      print "Feed Angle found :",FeedAngl

   #print 'debug: subscanToDo,subscan_range',subscanToDo,subscan_range
   ...
   for isub in range(1,SubScansInLoop):                # This is not 'where a float is required'
      print "Starting with Subscan ",isub," from ",SubScansInLoop
      oncePerSubscan=1
      midnight=False
      midnightJP=False
      ...
      if subs_type in ["OTF","otf","onTheFly"]:
         ...
         if seg_type == "linear":
            ...
            print "We have here segment 1",seg_xStart,seg_xEnd,seg_yStart,seg_yEnd,seg_speedStart,seg_speedEnd
            print "timePerSubscan",subs_timePerSubscan
         ...
         ##print obsBlock.scans.list[0].subscans.list[isub-1].presets.list[0].values['focusTranslation']
            ...
            print "No preset focusOffset found"
         ...
         try:
            ...
            print "No preset focusTranslation found"
...
##         findKey ="antMD:%s.%s.%s.%s:backOnTrack" % (Date,iScanNo,isub,iseg)
         ...
         findKey ="antMD:%s.%s.%s:subscanStarted" % (Date,iScanNo,isub)
      print 'To find dateObs use ',findKey
      found = syncMsg.has_key(findKey)
      if found:
         dateObs=syncMsg[findKey]
         print 'Sync message found: ',findKey,dateObs
      else:
         ...
         if subs_type in ["OTF","otf","onTheFly"]:
            ...
            if found:
               ...
               print 'EXIT: Sync message expected from: ',findKey
               OutFITSfile.close()
               sys.exit(1)
   ...
   ### Now the antMD table: Find the subscan start of backend integration
      ...
      if found and isub != 1:
         ...
         if found:
            ...
            print "Subscan Start not found ",isub,findKey
      print "Start of subscan found ",subScanStart," from ",syncMsg[findKey]
      if isub == 1:
...
##      elif subs_type in ["calPaddle","calCold","calAmbient","calSky","calGrid"]:   #Fix me
      ...
      if found:
         ...
         print "Date end H M S ",DatePart,endH,endM,endS,
         ...
         print "Subscan End not found ",isub,findKey
         break
      if isub == (SubScansInLoop-1):
         ScanEnd=subScanEnd
      
      print " to ",endH,endM,endS
      print "subscan",isub,"started",subScanStart,"ended",subScanEnd,findKey
      print "For the whole Scan we use as start ",ScanStart," and as end ",ScanEnd
      NoofBEpartsOut=8
      if isub == 1:
      ...
      if subs_type in ["OTF","otf","onTheFly"]:
         ...
         print "subs_systemOffset",subs_systemOffset
         Febepar4.BinTable.subsTableEntry_str(fptr,'SYSTEMOF',[subs_systemOffset])
         Febepar4.BinTable.subsTableEntry(fptr,'SETIME01',[string.atof(subs_timePerSubscan)])
         print  'seg_type', seg_type
         if seg_type == "linear":
            print 'seg_xStart seg_yStart seg_xEnd seg_yEnd seg_speedStart seg_speedEnd',seg_xStart,seg_yStart,seg_xEnd,seg_yEnd,seg_speedStart,seg_speedEnd
      ...
      if isub == (SubScansInLoop-1):
         ...
         writeDataStreams=1
         OBSnum=1
         print "Last Subscan  is ",isub
##         OutFITSfile.close()
##         exit()
      print "FINISHED ",isub," now write the data:",writeDataStreams
   if writeDataStreams:
# Detect Midnight 
      print SDate[-2:], '==', DatePart[-3:-1]
      if SDate[-2:] == DatePart[-3:-1]:
         midnight=False
         SDateM=SDate
            #In any case we need the date before the last midnight!
            #iMJDOBSbm=iMJDOBS-1
         print 'iMJDOBS',iMJDOBS,type(iMJDOBS)
         if passedMidnight:
            iMJDOBSbm=iMJDOBS
            print 'iMJDOBSbm pass',iMJDOBSbm
         else:
            iMJDOBSbm=iMJDOBS-1.0
            print 'iMJDOBSbm==',iMJDOBSbm, type(iMJDOBSbm)
         ...
         SDateBM=SDate
      print "Midnight detected ",midnight,SDate,SDateM,passedMidnight
...
###  Find the traces
      ...
      if midnight and timeFrom<timeTo:
         midnight=False
         midnightJP=True
         print "We just passed Midnight:",timeFrom,timeTo
      #
      # From
...
##      endSOD=endS+(endM+endH*60.)*60. + 2.
      ...
      print 'timeToFi ',timeToFi

      print timeFromM,timeFrom,timeTo,timeToFi
      ...
      if prev==1 and count>0:
         ...
         smallList=[prevFile]
      print count+prev,"antenna traces found:",smallList 
      print "Now try ",SDateBM
      if prev==0 and len(SDateBM)>1 and not midnight:
         ...
         print "Extended smallList ",smallList
      ...
      print "antmd MJDfrom, MJDto ",MJDfrom, MJDto
      print "antmd MJDfrom, MJDto ",timeFrom[0:2],timeFrom[3:5],timeFrom[-6:],timeFrom,isub
      print "antmd MJDfrom, MJDto ",timeTo[0:2],timeTo[3:5],timeTo[-6:],timeTo,
...
# New table for slow antMD traces
      for infile in smallList:
         ...
         nelements=numRows
         if oncePerSubscan:
            print 'Found traceRate ',traceRate,' <<<<<<<<<<<<<<<<<'
         ...
         print len(days1970),' How many slow traces ?'
         outFrom=0;outTo=0
         for ii in range(numRows):
            MJDin=days1970[ii]
            #print MJDin,MJDfrom,MJDto,ii
...
#     New From for fast traces
      ...
      timeFromM="%02.2d:%02.2d:%06.3f" % (endH,endM,endS)
      timeFromMSI="%02.2d%02.2d%06.3f" % (endH,endM,endS)
      print 'timeFromM ',timeFromM
#     To
      print "endSODft ",endSODft
      ...
      print "timeFromM,timeToFi :",timeFromM,timeToFi
      for infile in largeList:
         infileTime=infile[-11:-9]+":"+infile[-9:-7]+":"+infile[-7:-5]
         if midnight:# Midnight fixed?? 
            if infileTime>=timeFromM and infileTime<"24:00:00":
               count=count+1
               smallList.append(infile)
               print "m if infileTime,timeFromM,timeToFi ",infileTime,timeFromM,timeToFi  
            elif infileTime>="00:00:00" and infileTime<timeToFi:
               count=count+1
               smallList.append(infile)
               print "m elif infileTime,timeFromM,timeToFi ",infileTime,timeFromM,timeToFi  
         else:
            if infileTime<timeFromM:              ## Was set to timeFrom ??
               prevFile=infile
               prev=1
               #print infileTime,
               #print "if infileTime,timeFromM,timeToFi ",infileTime,timeFromM,timeToFi  
            elif infileTime>=timeFromM and infileTime<timeToFi:
               count=count+1
               smallList.append(infile)
               print "elif infileTime,timeFromM,timeToFi ",infileTime,timeFromM,timeToFi
      if prev==1 and count>0:
         smallList=(smallList[:0]+[prevFile])+smallList
      elif prev==1 and count==0:
         smallList=[prevFile]
      print count+prev,"antenna traces found f:",smallList 
      print "Now try ",SDateBM
      if prev==0 and len(SDateBM)>1 and not midnight:
         largeList=glob.glob(DataRootDir+"/datastreams/antmd/iram30m-antmd-"+SDateBM+"t2*")
         if len(largeList)==0:largeList=glob.glob(DataRootDir+SDateBM+"/datastreams/antmd/iram30m-antmd-"+SDateBM+"t2*")
         ...
         if len(smallList)==0:
            ...
            smallList=(smallList[:0]+[largeList[-1]])+smallList
         print "Extended smallList f ",smallList
...
##      # special cheat for rz
      ...
      for infile in smallList:
         ...
         slowRate=string.atoi(pcfitsio.fits_read_keyword(fptrin,'SLOWRATE')[0])
         nelements=numRows
         #print 'Fast amd oncePerSubscan ',oncePerSubscan
         if oncePerSubscan:
            print 'Found traceRate ',traceRate,' <<<<<<<<<<<<<<<<<'
            imbfits_NIKA.ComClass.TRACERAT=traceRate
            # Open DAP's table 1
         ...
         print 'No. of fast time-stamps ',len(days1970f)
         days1970ff=[]
...
#Bug                iii = iii+1
         #print 'Using ',len(days1970ff),' corresponding to the first fast trace of each slow trace.'
         outFrom=0;outTo=0
         for ii in range(numRows):
            MJDin=days1970ff[ii]
            #print MJDin,MJDfrom,MJDto,ii
         ...
         print 'nelements before ',nelements
         print 'nTr,mTr ',nTr,mTr
#         if (outTo<numRows and nelements>1):                  # Only when using the last file in the list. FIX ME
         if (nTr == mTr):
            if ((neleFast-neleSlow) > -3):
               ...
               #print " neleFS ",infile[-20:-4]," ,isub,neleSlow,neleFast,(neleFast-neleSlow),len(smallList)"
               print " neleFS ",infile[-20:-4]," ",isub,neleSlow,neleFast,(neleFast-neleSlow),len(smallList)
            else:
               print "No correction for: "
               print " neleFSn ",infile[-20:-4]," ",isub,neleSlow,neleFast,(neleFast-neleSlow),len(smallList)
         print 'nelements after ',nelements
   ...
   ##
      ...
      if prev==1 and count>0:
         ...
         smallList=[prevFile]
      print count+prev,"subreflector traces found:",smallList
      ...
      for infile in smallList:
            ...
            if midnight:
               ...
               if ii>0 and UCTms[ii]<UCTms[ii-1]:
                  ...
                  if outTo > 0:
                     ...
                     outFrom=ii
                  print "after midnight found: ",afterMid,ii,outTo
         ...
         print 'Use lines ',outFrom,outTo,' from ',infile
         # If all traces have time stamps >= MJDto
   ...
   ### Preparing to write the SIS continuum data
      if ChosenBackend=='continuum':
         ...
         if len(largeList) == 0:
            print "No data found for continuum"
         largeList.sort(lambda x, y: cmp(x[-20:],y[-20:]))
         smallList=[]
         prev=0;count=0
   ...
   #      timeFromSI=(string.atoi(timeFrom[:2]) * 60 + string.atoi(timeFrom[3:5]))*60 + string.atoi(timeFrom[6:8])
         ...
         print 'time(From,To)SI ',timeFromSI,timeToSI
         ...
         if prev==1 and count>0:
            ...
            smallList=[prevFile]
         print count+prev,"continuum data found:",smallList
   ...
   # The raw data always has 4 channels, but we may have used less
            ...
            #print 'Original outFrom outTo ',outFrom,outTo
            notIn=0
            # If all traces have time stamps > MJDto
            if outFrom == 0 and outTo == 0 : continue
            ...
            if outTo > numRows:
               outTo=numRows
            #print 'New outFrom outTo ',outFrom,outTo
            ...
            if (outTo-outFrom) >= 0 and notIn == 0:
               for idap in range(outFrom,outTo+1):
                  ...
                  if math.fabs(problem)>0.002:
                     print "Problem in 2KHz Counter",problem
      ...
      if ChosenBackend=='NIKA1mm' or ChosenBackend=='NIKA2mm':
         ...
         if ChosenBackend=='NIKA1mm':
            ...
            if os.access(inDirBackend+NIKAfile,modeAccess):
               print 'Found:', NIKAfile,' from ',inDirBackend
            else:
               ...
               if os.access(inDirBackend+NIKAfile,modeAccess):
                  ...
                  if os.access(inDirBackend+NIKAfile,modeAccess):
                     ...
                     print 'Finally give up',inDirBackend,NIKAfile
...
#         timeTo=subScanEnd[0:8]
         ...
         timeToSI=timeTo[:2] + timeTo[3:5] + timeTo[6:8]
         secondsTo=(string.atoi(timeTo[:2]) * 60 + string.atoi(timeTo[3:5]))*60 + string.atoi(timeTo[6:8])
         print 'time(From,To)SI ',timeFromSI,timeToSI,secondsFrom,secondsTo
         ...
         MJDto=sla.mjd(iYear,iMonth,iDay)+((string.atoi(timeTo[0:2])*60.+ string.atoi(timeTo[3:5]))*60.+ string.atof(timeTo[-6:])+0.0)/86400.
         if midnight:MJDfrom=MJDfrom-1.0 #Fix me 20060908
         print MJDfrom, MJDto,' NIKA backend'
         print "Extended smallList ",smallList
...
# First write a table with tuning information
               if NIKAConfigWrite:
                  ...
                  if numPixels != usedchan:
                     usedchan=numPixels
                     print 'No. of pixel changed to ',numPixels
                     Febepar1.Header.updateKeyword(fptr,'NUSEFEED','',usedchan)
                     Febepar1.Header.updateKeyword(fptr,'FEBEFEED','',usedchan)
                     Febepar2.Header.updateKeyword(fptr,'NUSEFEED','',usedchan)
                     Febepar2.Header.updateKeyword(fptr,'FEBEFEED','',usedchan)
...
#               Arraydata1.Header.updateKeyword(fptr,'CHANNELS','',usedchan*bePixels)
            ...
            column=2 ## MJD
            days1970=pcfitsio.fits_read_col(fptrin,column,firstrow,firstelem,nelements,nulval)[:-1]
            print len(days1970),' Backend NIKA timestamps'
...
#                  print idap+1,' data_r ',data_r
#                  print idap+1,' data_i ',data_i
#                  print idap+1,' data_q ',data_q
...
#         print Sample
         ...
         print 'numPixels samples usedchan ',numPixels,nb_samples,usedchan
         imbfits_NIKA.ComClass.NCH=usedchan
...
#         outSample=[]
         ...
         print outSample[0],outSample[nb_samples-1]
# MJD now
...
##          Arraydata1.BinTable.subsTableEntry_dbl(fptr,'DFTONE',DFTff)
            ...
            Arraydata1.BinTable.subsTableEntry_dbl(fptr,'R',    RFff)
#           Only 1 phase please 
...
#            print 'Cumulative Lines antenna slow, fast : ',totAnts,totAntf
#         if WichExt[1:13]=='IMBF-subrefl':
#            totSubr=totSubr + naxis2
#         if WichExt[1:13]=='IMBF-backend':
#            totBackend=totBackend + naxis2
#   print 'Cumulative Line count tot : ',totAnts,totAntf,totSubr,totBackend         
#   pcfitsio.fits_movabs_hdu(fptr,1) # Main header
...
#   pcfitsio.fits_update_key(fptr,'TOTBACK',totBackend,'Total no. of LINES in backenddata tables')
   ...
   if offLine:
      ...
      print 'Reached end of script',sys.argv[0]
