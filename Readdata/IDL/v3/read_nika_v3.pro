...
;
FUNCTION READ_NIKA_v3, file, param_c, param_d, data, dataU, $ ;periode, $
...
libso = compilation_dir + "IDL_read_data_v3.so"
...
nb_read_samples = call_external( libso, 'IDL_read_infos', file_tmp, buffer_header, $
                                 ...
                                 list_data_tmp, code_list_detector, list_detector,$
                                 nb_Sc,nb_Sd,nb_Uc,nb_Ud,$
                                 idx_param_c, idx_param_d, silent)
...
if (nb_data_Sc gt 0) then begin
   nom_data_Sc = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_Sc-1]
   idxinit += nb_char_nom*nb_data_Sc
                                ;unites_data_c = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_communs-1]
                                ;idxinit += nb_char_nom*nb_data_communs
endif

if (nb_data_Sd gt 0) then begin
   nom_data_Sd = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_Sd-1]
   idxinit += nb_char_nom*nb_data_Sd
                                ;unites_data_d = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_detecteurs-1]
                                ;idxinit += nb_char_nom*nb_data_detecteurs
endif

if (nb_data_Uc gt 0) then begin
   nom_data_Uc = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_Uc-1]
   idxinit += nb_char_nom*nb_data_Uc
                                ;unites_data_c = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_communs-1]
                                ;idxinit += nb_char_nom*nb_data_communs
endif

if (nb_data_Ud gt 0) then begin
   nom_data_Ud = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_Ud-1]
   idxinit += nb_char_nom*nb_data_Ud
                                ;unites_data_d = nom_var_all[idxinit:idxinit+nb_char_nom*nb_data_detecteurs-1]
                                ;idxinit += nb_char_nom*nb_data_detecteurs
endif
...
; Data undersample
strexc = 'dataU ={'
if (nb_data_Uc gt 0 or nb_data_Ud gt 0) then begin
    strexc=strexc + 'sampleU: 0.d0, '
endif

if (nb_data_Uc gt 0) then begin
   for idx=0,nb_data_Uc-1 do begin
      if not keyword_set(silent) then print, 'dcommuns box parameter : '+ convert_fromc_string(nom_data_Uc[idx*nb_char_nom:*])
;;      strexc=strexc + convert_fromc_string(nom_data_c[idx*nb_char_nom:*]) +': 0.d0,'
      mytag = strtrim( strupcase(convert_fromc_string(nom_data_Uc[idx*nb_char_nom:*])), 2)
      print, convert_fromc_string(nom_data_Uc[idx*nb_char_nom:*])
 
      if (nb_data_Uc eq 1) and (nb_data_Ud eq 0) then begin
...
if (nb_data_Ud gt 0) then begin
   for idx=0, nb_data_Ud-1 do begin
      mytag = strtrim( strupcase(convert_fromc_string(nom_data_Ud[idx*nb_char_nom:*])), 2)
      if (nb_data_Ud eq 1) then begin 
...
strexc = 'data ={'
if (nb_data_Sc gt 0) then begin
   for idx=0,nb_data_Sc-1 do begin
      if not keyword_set(silent) then print, 'dcommuns box parameter : '+ convert_fromc_string(nom_data_Sc[idx*nb_char_nom:*])
;;      strexc=strexc + convert_fromc_string(nom_data_c[idx*nb_char_nom:*]) +': 0.d0,'
      mytag = strtrim( strupcase(convert_fromc_string(nom_data_Sc[idx*nb_char_nom:*])), 2)
      print, convert_fromc_string(nom_data_Sc[idx*nb_char_nom:*])
...
if (nb_data_Sd gt 0) then begin
   for idx=0, nb_data_Sd-1 do begin
      mytag = strtrim( strupcase(convert_fromc_string(nom_data_Sd[idx*nb_char_nom:*])), 2)
...
length_data_per_sample = nb_data_Sc + nb_data_Sd*n_det
length_data_per_sampleU = nb_data_Uc + nb_data_Ud*n_det
...
;print, nb_samples_request/nb_pt_bloc*length_data_per_sampleU

bufferdata = dblarr(nb_samples_request*length_data_per_sample)

if (length_data_per_sampleU gt 0) then begin
 bufferdataU = dblarr(nb_samples_request/nb_pt_bloc*length_data_per_sampleU)
endif else begin
 bufferdataU = dblarr(2)
endelse
...
list_data_tmp = convert_toc_string(list_data)
nb_samples_lu = call_external( libso,'IDL_read_data', file_tmp, bufferdata, bufferdataU, $
...
if (length_data_per_sampleU gt 0) then begin
   bufferdataU = reform(bufferdataU[0L:length_data_per_sampleU*nb_bloc_lu-1l],length_data_per_sampleU,nb_bloc_lu)
endif
...
for idx=0,nb_data_Sc-1 do $
   data.(idx) = reform(bufferdata[idx,*], nb_samples_lu)
      
;; ----------  les  data  detecteurs  -----------------
for idx=0,nb_data_Sd-1 do begin
   data.(idx+nb_data_Sc) = (bufferdata[nb_data_Sc+idx*n_det: nb_data_Sc+idx*n_det+n_det-1,*])
endfor


if (nb_data_Uc gt 0 or nb_data_Ud gt 0) then begin
   dataU = replicate( dataU, nb_bloc_lu)
endif

if (nb_data_Uc gt 0 or nb_data_Ud gt 0) then begin
   dataU.(0) = dindgen(nb_bloc_lu)
endif 

for idx=0,nb_data_Uc-1 do $
   dataU.(idx+1) = reform(bufferdataU[idx,*], nb_bloc_lu)
      
;; ----------  les  data  detecteurs  -----------------
for idx=0,nb_data_Ud-1 do begin
   dataU.(idx+nb_data_Uc+1) = (bufferdataU[nb_data_Uc+idx*n_det: nb_data_Uc+idx*n_det+n_det-1,*])
endfor
